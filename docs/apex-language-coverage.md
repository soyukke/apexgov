...
| Apex-to-Java transpile scaffold (`emulate transpile`) | partial | `.cls` を Java クラス骨組みに変換し、`@IsTest` を `@Test` 化。`System.assert*` は `SystemAssert.*` へ変換。メソッド本体は基本コメント埋め込み（手動移植前提） |
