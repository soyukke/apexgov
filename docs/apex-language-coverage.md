...
| async flush at `stopTest()` | supported | `@Future`, Queueable, Batchable, Schedulable を順次実行（Batchable は `QueryLocatorBatchable` の scope 分割実行にも対応） |
...
| SOQL subset query | partial | `FROM` / `[SELECT ...]`, `WHERE` (`AND`/`OR`/unary `NOT` with grouped expression, `= != > >= < <=`, `IN`, `NOT IN`, `LIKE` with escape), `ORDER BY` multi-key + `NULLS FIRST/LAST` (default mode configurable: `FIRST`/`LAST`/`DIRECTIONAL`), `LIMIT`, `queryWithBinds/countQueryWithBinds` (`:name`, scalar + collection bind), `getQueryLocator/getQueryLocatorWithBinds` |
