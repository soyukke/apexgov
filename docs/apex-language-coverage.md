...
| `switch on / when` | partial | transpile は literal列挙 + `when else` を Java `switch` へ変換。型パターン `when Account a` は未対応 |
