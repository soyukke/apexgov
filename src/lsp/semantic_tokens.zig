...
fn classifyToken(tok: Token) ?u32 {
    return switch (tok.kind) {
        ...
        // 演算子
        .plus, .minus, .star, .slash, .percent, .eq, .neq, .lt, .gt, .lte, .gte, .strict_eq, .strict_neq, .and_op, .or_op, .not_op, .assign, .plus_assign, .minus_assign, .star_assign, .slash_assign, .arrow, .question, .question_dot => TT.operator,
