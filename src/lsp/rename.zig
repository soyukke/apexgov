...
test "renames variable at all sites" {
    ...
    const edit = try getRenameEdits(&br, source, "file:///t.cls", sym.loc.offset, "newName", alloc);
...
test "unknown position returns null" {
    ...
    const edit = try getRenameEdits(&br, source, "file:///t.cls", 6, "x", alloc);
    try std.testing.expect(edit == null);
}
