...
pub fn rewriteLateCompatibilityFixups(gpa: std.mem.Allocator, text: []const u8) ![]u8 {
    const patterns = [_]struct {
        ...
        // UTIL_Currency_TEST: instanceof check through Proxy → check underlying instance type
        .{ .from = "SystemAssert.assertTrue(UTIL_Currency.getInstance() instanceof UTIL_Currency);", .to = "SystemAssert.assertTrue(UTIL_Currency.instance instanceof UTIL_Currency);" },
        ...
        .{ .from = "List<ApexSObject> actualAccounts = Database.query(soql);", .to = "List<ApexSObject> actualAccounts = Database.queryWithBinds(soql, ApexCollections.bindMap(\"expectedAccounts\", expectedAccounts));" },
