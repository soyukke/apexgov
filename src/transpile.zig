...
fn baseExprLikelyString(base_expr: []const u8, string_names: []const []u8) bool {
...
fn rewriteBrokenZeroLengthListInitializers(gpa: std.mem.Allocator, text: []const u8) ![]u8 {
    var out: std.ArrayList(u8) = .empty;
    errdefer out.deinit(gpa);
...
fn rewriteQuerySingletonCallsAssignedToLists(gpa: std.mem.Allocator, text: []const u8) ![]u8 {
    var out: std.ArrayList(u8) = .empty;
    errdefer out.deinit(gpa);
...
fn rewriteQuerySingletonAssignmentsToDeclaredListVars(gpa: std.mem.Allocator, text: []const u8) ![]u8 {
...
fn rewriteDeclaredListQuerySingletonLine(gpa: std.mem.Allocator, line: []const u8, list_name: []const u8) !?[]u8 {
    const prefixes = [_][]const u8{
        "ApexCollections.firstOrNull(",
        "ApexCollections.firstOrThrow(",
    };
...
fn getAsCallNeedsNumericCompatibility(gpa: std.mem.Allocator, line: []const u8, call_text: []const u8, double_names: []const []u8) bool {
...
fn extractGetAsCallStringLiteralFieldName(call_text: []const u8) ?[]const u8 {
    const open = blk: {
...
fn fieldNameLooksNumeric(field_name: []const u8) bool {
    return containsIgnoreCaseSubstring(field_name, "amount") or
        ...
        containsIgnoreCaseSubstring(field_name, "number");
}
...
fn rewriteGetAsFieldAddErrorCalls(gpa: std.mem.Allocator, text: []const u8) ![]u8 {
    var out: std.ArrayList(u8) = .empty;
    errdefer out.deinit(gpa);
...
fn rewriteBooleanEqualsIsEmptyArtifacts(gpa: std.mem.Allocator, text: []const u8) ![]u8 {
    var out: std.ArrayList(u8) = .empty;
    errdefer out.deinit(gpa);
...
fn rewriteTrailingDatabaseQueryAssignmentParens(gpa: std.mem.Allocator, text: []const u8) ![]u8 {
    var out: std.ArrayList(u8) = .empty;
    errdefer out.deinit(gpa);
    var replaced = false;
...
fn normalizeDatabaseQueryAssignmentLine(gpa: std.mem.Allocator, line: []const u8) !?[]u8 {
    const method = blk: {
...
fn rewriteListMethodQuerySingletonReturns(gpa: std.mem.Allocator, text: []const u8) ![]u8 {
    var out: std.ArrayList(u8) = .empty;
    errdefer out.deinit(gpa);
    var replaced = false;
...
fn countByte(text: []const u8, needle: u8) isize {
    var count: isize = 0;
    for (text) |ch| {
...
fn isListMethodSignatureLine(line: []const u8) bool {
    if (line.len == 0 or line[line.len - 1] != '{') return false;
    if (startsWithWordIgnoreCase(line, "if") or
        startsWithWordIgnoreCase(line, "for") or
        startsWithWordIgnoreCase(line, "while") or
...
fn normalizeListMethodQuerySingletonReturnLine(gpa: std.mem.Allocator, line: []const u8) !?[]u8 {
    const prefixes = [_][]const u8{
...
fn assignmentContextExpectsBoolean(text: []const u8, eq_idx: usize) bool {
