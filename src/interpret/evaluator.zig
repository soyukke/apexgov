...
pub const Evaluator = struct {
    ...
    next_id: u64 = 1,
    /// Id → SObject type_name mapping (populated by insert_record and createId)
    ...
    // Property getter currently being evaluated (to prevent infinite recursion in self-referencing
    // getters)
    ...
    trigger_depth: u32 = 0,
    // Cast type hints for method overload resolution (set by eval_method_call, consumed by
    // find_best_method_in_class_filtered)
    ...
    // Line number of the current call-site expression (set before call_method/call_instance_method)
    ...
    pub fn reset_for_test(self: *Evaluator) void {
    ...
    fn create_default_user_record(self: *Evaluator) !Value {
        if (self.store.get("User")) |users| {
            for (users.items) |existing| {
                ...
                if (std.ascii.eqlIgnoreCase(existing.sobject.id.?, "005000000000001")) {
                    const clone = try self.clone_s_object(existing.sobject);
                    return Value{ .sobject = clone };
                }
            }
        }
        return self.create_builtin_user_record(
            "005000000000001",
            "00e000000000001",
    ...
    /// Create a synthetic User record for UserInfo.getUserId() — used by SOQL when no User records
    /// exist in store
    pub fn create_current_user_record(self: *Evaluator) !Value {
        ...
        if (std.ascii.eqlIgnoreCase(self.current_user_id, "005000000000001")) {
            return self.create_default_user_record();
        }
        return self.create_builtin_user_record(
    ...
    fn should_synthesize_builtin_user(self: *Evaluator, soql: []const u8, current_env: *Env) bool {
    ...
    fn current_user_field_value(
        ...
        if (self.store.get("User")) |users| {
            for (users.items) |existing| {
                ...
                if (utils.sobject_get(&existing.sobject.fields, field_name)) |field_value| {
    ...
    fn query_matches_current_user(self: *Evaluator, soql: []const u8, current_env: *Env) bool {
        if (self.extract_where_field_value(soql, "Id", current_env)) |user_id| {
            if (std.ascii.eqlIgnoreCase(user_id, self.current_user_id)) return true;
        }
        if (self.extract_where_field_value(soql, "Alias", current_env)) |alias| {
            const current_alias = self.current_user_field_value("Alias", "tuser");
    ...
    pub fn create_current_profile_record(self: *Evaluator) !Value {
        const profile = try self.arena.create(types.SObject);
    ...
    /// Create a synthetic Profile record — used by SOQL when no Profile records exist in store
    pub fn create_default_profile_record(self: *Evaluator) !Value {
        return self.create_current_profile_record();
    }
    ...
    fn create_profile_for_query(self: *Evaluator, soql: []const u8, current_env: *Env) !Value {
        if (self.extract_where_field_value(soql, "Id", current_env)) |profile_id| {
        ...
        const profile_name = self.extract_where_field_value(
            soql,
            "Name",
            current_env,
        ) orelse "System Administrator";
        const profile_id = try self.alloc_id();
        profile.id = profile_id;
        try profile.fields.put(self.arena, "Id", Value{ .string = profile_id });
        try self.populate_synthetic_profile(profile, profile_name);
        try self.apply_queried_synthetic_profile_flags(profile, soql, current_env);
        return Value{ .sobject = profile };
    }
    ...
    fn apply_queried_synthetic_profile_flags(
        ...
        while (pos < where_clause.len) : (pos += 1) {
            ...
            if (self.extract_where_field_value(soql, field_name, current_env)) |raw_value| {
    ...
    fn find_profile_by_name(self: *Evaluator, profile_name: []const u8) ?*types.SObject {
        if (self.store.get("Profile")) |profiles| {
            for (profiles.items) |record| {
                if (record != .sobject) continue;
                if (utils.sobject_get(&record.sobject.fields, "Name")) |name| {
    ...
    fn is_guest_user_id(self: *Evaluator, user_id: []const u8) bool {
        if (self.store.get("User")) |users| {
            for (users.items) |record| {
                ...
                if (utils.sobject_get(&record.sobject.fields, "UserType")) |user_type| {
                    if (user_type == .string and std.ascii.eqlIgnoreCase(user_type.string, "Guest"))
                        return true;
                }
    ...
    fn collapse_like_wildcards(self: *Evaluator, pattern: []const u8) []const u8 {
    ...
    fn create_user_for_query(self: *Evaluator, soql: []const u8, current_env: *Env) !Value {
        if (self.query_matches_default_synthetic_user(soql, current_env)) {
            return self.create_default_user_record();
        }
        ...
        const username = self.extract_where_field_value(
            ...
            "Username",
            current_env,
        ) orelse if (std.ascii.startsWithIgnoreCase(alias, "autoproc"))
        ...
        switch (value) {
            ...
            .list => |list| {
                for (list.items.items) |item| {
                    try self.append_record_ids_from_value(item, record_ids);
                }
            },
            .set => |set| {
            ...
            .map => |map| {
                ...
                const coerced = utils.coerce_to_string(value, self.arena) catch return;
                if (coerced.len > 0) try record_ids.append(self.arena, coerced);
            },
        }
    }

    fn can_delete_record_via_user_record_access(self: *Evaluator, record_id: []const u8) bool {
        const type_name = self.find_record_type_by_id(record_id) orelse blk: {
            if (record_id.len < 3) break :blk "SObject";
            break :blk sobject_type_from_prefix(record_id[0..3]);
        };
        if (std.ascii.eqlIgnoreCase(type_name, "SObject")) return false;
        if (std.ascii.eqlIgnoreCase(type_name, "User")) return false;
        if (self.is_setup_object(type_name)) return false;
        return true;
    }
    ...
    fn collect_user_record_access_ids(
        ...
        if (record_ids.items.len == 0) {
            if (self.extract_where_field_value(soql, "RecordId", current_env)) |record_id| {
    ...
    /// PermissionSetLicense, etc.).
    /// Extracts names from WHERE Name/DeveloperName IN (:var) or IN ('a','b') and creates a stub
    /// for each.
    fn seed_named_records(
        ...
        const where_clause = extract_where_clause(soql) orelse return;
        // Collect names from the IN clause — supports both bind variables (Set/List) and literal
        // lists
        ...
        if (std.ascii.indexOfIgnoreCase(where_clause, " IN :")) |in_pos| {
            ...
            if (j > start) {
                ...
                if (self.lookup_bind_value(current_env, var_name)) |val| {
                    if (val == .set) {
        ...
        for (names.items) |name| {
            ...
            const stub_id = try self.alloc_id();
            stub.id = stub_id;
    ...
    /// IDs are kept in sync with create_record_type_info/create_describe_result in builtins.zig.
    fn seed_record_type_store(self: *Evaluator) !void {
    ...
    fn convert_picklist_values(self: *Evaluator, obj: *types.SObject) !void {
        ...
        for (obj.fields.keys(), obj.fields.values()) |field_name, *field_val| {
            ...
            if (self.resolve_picklist_label(obj.type_name, field_name, field_val.string)) |label| {
    ...
    pub fn re_init_class_static_fields(self: *Evaluator, cd: *ast.ClassDecl) void {
    ...
    /// Resolve a class name to its fully-qualified form (e.g., "InnerClass" →
    ...
    fn simple_class_name(name: []const u8) []const u8 {
        if (std.mem.lastIndexOfScalar(u8, name, '.')) |di| return name[di + 1 ..];
        return name;
    }

    fn has_top_level_class(self: *Evaluator, name: []const u8) bool {
    ...
    fn resolve_visible_user_class_in_scope(
        ...
        if (scope_class) |scope_name| {
            if (std.ascii.eqlIgnoreCase(simple_class_name(scope_name), simple_name)) {
                return scope_name;
            }
            if (self.find_inner_class_fq(scope_name, simple_name)) |fq| return fq;
            if (self.find_outer_class_name(scope_name)) |outer_name| {
                if (std.ascii.eqlIgnoreCase(simple_class_name(outer_name), simple_name)) {
                    return outer_name;
                }
                if (self.find_inner_class_fq(outer_name, simple_name)) |fq| return fq;
            }
        }

        if (self.has_top_level_class(simple_name)) return simple_name;
        return null;
    }

    fn is_builtin_static_namespace(simple_name: []const u8) bool {
    ...
    pub fn build_stack_trace_string(self: *Evaluator) ![]const u8 {
    ...
    pub fn run_class_static_inits(self: *Evaluator, cd: *ast.ClassDecl) void {
        const saved_class = self.current_class;
        self.current_class = cd.name;
        defer self.current_class = saved_class;
    ...
    pub fn load_decls(self: *Evaluator, decls: []const ast.Decl) anyerror!void {
        ...
        // are evaluated lazily via ensure_static_init() on first class access.
    ...
    pub fn run_static_inits(self: *Evaluator) void {
    ...
    pub fn call_method(
        ...
        try self.call_stack.append(
            self.arena,
            .{ .class_name = class_name, .method_name = method_name, .line = frame_line },
        );
            ...
            std.ascii.eqlIgnoreCase(method_name, "publish"))
        {
            ...
            const publish_one = struct {
                fn run(self_eval: *Evaluator, item: Value) !PublishResult {
                    ...
                    self_eval.insert_record(item.sobject) catch |err| {
            ...
            const result = if (args.len > 0 and args[0] == .list) blk: {
                ...
                const publish_result: PublishResult = if (args.len > 0) try publish_one(
                    self,
                    args[0],
                ) else .{ .success = false, .id = null };
            ...
            // If a callback is provided (second arg), store it for later processing by
            // Test.getEventBus()
            if (args.len >= 2 and args[1] == .object and any_publish_success) {
                ...
                if (self.find_class(callback.class_name)) |cb_class| {
                    ...
                    if (args[0] == .sobject) {
                        if (utils.sobject_get(&args[0].sobject.fields, "EventUuid")) |uuid_val| {
                            try uuid_list.items.append(self.arena, uuid_val);
                        }
                    }
                    try pub_result.fields.put(self.arena, "eventUuids", Value{ .list = uuid_list });
                    // Call onSuccess(result) on the callback (will be overridden by fail() if
                    // called)
        ...
        if ((std.ascii.eqlIgnoreCase(class_name, "Search") or std.ascii.eqlIgnoreCase(
            class_name,
            "System.Search",
        )) and
        ...
        while (iter.next()) |entry| {
            if (std.ascii.eqlIgnoreCase(entry.key_ptr.*, class_name)) {
                ...
                // Try type-aware resolution first (prefer static methods for call_method)
                    ...
                    switch (member) {
                        .enum_decl => |ed| {
                            if (std.ascii.eqlIgnoreCase(ed.name, class_name)) {
                                if (std.ascii.eqlIgnoreCase(method_name, "valueOf") and
                                    args.len > 0 and
                                    args[0] == .string)
                                {
                                    for (ed.values) |v| {
                                        if (std.ascii.eqlIgnoreCase(v, args[0].string))
                                ...
                                for (ed.values) |v| {
                                    if (std.ascii.eqlIgnoreCase(v, method_name))
                                        return Value{ .string = v };
                                }
                            }
                        },
        ...
        if (try self.handle_test_factory(class_name, method_name, args)) |result| {
            return result;
        }
    ...
    fn try_switch_value_match(
        ...
        if (self.switch_type_pattern_name(&val_copy)) |type_name| {
            if (subject == .sobject and
                std.ascii.eqlIgnoreCase(subject.sobject.type_name, type_name))
            {
                return true;
            }
            if (subject == .object) {
                const subject_simple = if (std.mem.lastIndexOfScalar(
                    u8,
                    subject.object.class_name,
                    '.',
                )) |di| subject.object.class_name[di + 1 ..] else subject.object.class_name;
            ...
            switch (clause.pattern) {
                .values => |values| {
                    for (values) |val_expr| {
                        ...
                        if (self.try_switch_value_match(subject, val_copy, when_val)) {
    ...
    fn find_matching_catch(
        ...
        for (catches) |*cc| {
            ...
            const catch_simple = if (std.mem.lastIndexOfScalar(u8, catch_type, '.')) |di|
                catch_type[di + 1 ..]
            else
                catch_type;
            const exc_simple = if (std.mem.lastIndexOfScalar(u8, exc_class_name, '.')) |di|
                exc_class_name[di + 1 ..]
            else
                exc_class_name;
            ...
            if (self.find_class(exc_class_name)) |exc_cd| {
                if (exc_cd.super_class) |sc| {
                    const sc_simple = if (std.mem.lastIndexOfScalar(u8, sc.name, '.')) |di|
                        sc.name[di + 1 ..]
                    else
                        sc.name;
        ...
        return matched_catch orelse generic_catch;
    }

    fn exec_try_stmt(self: *Evaluator, ts: *ast.TryStmt, current_env: *Env) anyerror!StmtResult {
        ...
        if (result) |r| {
            ...
            const selected = self.find_matching_catch(ts.catches, exc_class_name) orelse
                &ts.catches[0];
    ...
    fn iterable_is_list_of_lists(iterable: Value) bool {
        if (iterable.list.element_type) |et| {
            if (std.ascii.startsWithIgnoreCase(et, "List")) return true;
            if (std.ascii.startsWithIgnoreCase(et, "Set")) return true;
            if (std.ascii.startsWithIgnoreCase(et, "Map")) return true;
        }
        for (iterable.list.items.items) |item| {
            switch (item) {
                .list, .set, .map => return true,
                else => {},
            }
            break;
        }
    ...
    fn exec_for_each_list(
        self: *Evaluator,
        fes: anytype,
        iterable: Value,
        current_env: *Env,
    ) anyerror!StmtResult {
        const elem_type_is_list = std.ascii.eqlIgnoreCase(fes.elem_type.name, "List") and
            fes.elem_type.params.len > 0;
        const is_list_type = elem_type_is_list and !iterable_is_list_of_lists(iterable);
        if (is_list_type) {
            return try self.exec_for_each_chunked_list(fes, iterable, current_env);
        }
        const loop_env = try current_env.child();
        for (iterable.list.items.items) |item| {
    ...
    fn exec_for_each_object(
        ...
        const iterator_obj = blk: {
            if (self.find_class(iterable.object.class_name)) |cd| {
                const iter_val = self.call_instance_method(
                    cd,
                    iterable.object,
                    "iterator",
                    &.{},
                ) catch break :blk iterable;
                break :blk iter_val;
            }
            break :blk iterable;
        };
    ...
    fn exec_for_each_stmt(self: *Evaluator, fes: anytype, current_env: *Env) anyerror!StmtResult {
        ...
        return switch (iterable) {
            .list => try self.exec_for_each_list(fes, iterable, current_env),
            .set => try self.exec_for_each_set(fes, iterable, current_env),
            .map => try self.exec_for_each_map(fes, iterable, current_env),
            .object => try self.exec_for_each_object(fes, iterable, current_env),
            else => .normal,
        };
    }
    ...
    pub fn exec_stmt(self: *Evaluator, stmt: ast.Stmt, current_env: *Env) anyerror!StmtResult {
        switch (stmt) {
            ...
            .switch_stmt => |sw| return try self.exec_switch_stmt(sw, current_env),
            .try_stmt => |ts| return try self.exec_try_stmt(ts, current_env),
            .throw_stmt => |ts| {
    ...
    pub fn execute_dml(self: *Evaluator, op: ast.DmlOp, target: Value) anyerror!void {
        try self.execute_dml_with_external_id_internal(op, target, null, true, null);
    }
    ...
    fn execute_dml_with_external_id(
        ...
        try self.execute_dml_with_external_id_internal(op, target, external_id_field, true, null);
    }
        ...
        while (store_iter.next()) |entry| {
            ...
            for (entry.value_ptr.items) |record| {
                ...
                if (existing_value != .null_val and
                    self.field_values_equal_with_metadata(metadata, existing_value, field_value))
                {
                    return record.sobject;
                }
            }
    ...
    fn will_upsert_create_record(
        ...
        if (external_id_field) |field_name| {
            ...
            if (field_value != .null_val and
                self.find_record_by_field_value(obj.type_name, field_name, field_value) != null)
            {
                return false;
            }
        }
    ...
    pub fn get_user_profile_name(self: *Evaluator, user: *types.SObject) ?[]const u8 {
        ...
        if (utils.sobject_get(
            &user.fields,
            "profileId",
        ) orelse utils.sobject_get(&user.fields, "ProfileId")) |pid| {
    ...
    fn insert_record(self: *Evaluator, obj: *types.SObject) anyerror!void {
        try self.reject_insert_with_id(obj);
        try self.apply_missing_field_defaults(obj);
        if (try self.validate_required_fields(obj, false)) |err_msg| {
            return self.throw_dml_exception(err_msg);
        }

        const id = try self.assign_new_sobject_id(obj);
        const generated_flags = self.snapshot_generated_audit_flags(obj);
        const now_str = builtins.current_date_time_string(self.arena) catch "2026-01-01T00:00:00Z";
        try self.stamp_insert_audit_fields(obj);
        try self.synthesize_default_name_field(obj, id);
        try self.stamp_default_owner(obj);
        try self.resolve_relationship_foreign_keys(obj);
        ...
        const snapshot = try self.append_snapshot_to_store(obj, id, now_str, generated_flags);
        try self.apply_insert_type_specific_defaults(obj, snapshot, id);
    }

    fn reject_insert_with_id(self: *Evaluator, obj: *types.SObject) anyerror!void {
        const msg = "INVALID_FIELD_FOR_INSERT_UPDATE: cannot specify Id in an insert call";
        if (obj.id != null) return self.throw_dml_exception(msg);
        if (utils.sobject_get(&obj.fields, "Id")) |id_val| {
            if (id_val != .null_val) return self.throw_dml_exception(msg);
        }
    }

    /// Auto-assign an Id using Salesforce-style key prefixes, store the
    /// Id → type_name mapping for `getSObjectType()` lookups, and return it.
    fn assign_new_sobject_id(self: *Evaluator, obj: *types.SObject) ![]const u8 {
    ...
    const InsertAuditFlags = struct {
        created_date: bool,
        last_modified_date: bool,
        system_modstamp: bool,
    };

    fn snapshot_generated_audit_flags(
        _: *Evaluator,
        obj: *types.SObject,
    ) InsertAuditFlags {
        return .{
            .created_date = utils.sobject_get(&obj.fields, "CreatedDate") == null,
            .last_modified_date = utils.sobject_get(&obj.fields, "LastModifiedDate") == null,
            .system_modstamp = utils.sobject_get(&obj.fields, "SystemModstamp") == null,
        };
    }

    fn stamp_insert_audit_fields(self: *Evaluator, obj: *types.SObject) !void {
        ...
        if (utils.sobject_get(&obj.fields, "CreatedBy") == null) {
            try obj.fields.put(self.arena, "CreatedBy", try self.make_test_user_sobject());
        }
        ...
        if (utils.sobject_get(&obj.fields, "LastModifiedBy") == null) {
            try obj.fields.put(self.arena, "LastModifiedBy", try self.make_test_user_sobject());
        }
    }

    fn make_test_user_sobject(self: *Evaluator) !Value {
        const u = try self.arena.create(types.SObject);
        u.* = .{ .type_name = "User", .id = "005000000000001" };
        try u.fields.put(self.arena, "Id", Value{ .string = "005000000000001" });
        try u.fields.put(self.arena, "Name", Value{ .string = "Test User" });
        try u.fields.put(self.arena, "Username", Value{ .string = "testuser@example.com" });
        return Value{ .sobject = u };
    }

    fn synthesize_default_name_field(
        self: *Evaluator,
        obj: *types.SObject,
        id: []const u8,
    ) !void {
        ...
        if (existing_name != null and existing_name.? == .null_val) {
    ...
    fn stamp_default_owner(self: *Evaluator, obj: *types.SObject) !void {
    ...
    /// Resolve each `FieldName = someSObject` mapping into its corresponding
    /// `FieldNameId` foreign key, following external-Id lookups when the
    /// referenced record has no Id yet.
    fn resolve_relationship_foreign_keys(
        self: *Evaluator,
        obj: *types.SObject,
    ) !void {
        ...
        for (obj.fields.keys(), obj.fields.values()) |k, v| {
            if (v != .sobject) continue;
            if (rel_count >= rel_keys_buf.len) continue;
            const fk_name = try std.fmt.allocPrint(self.arena, "{s}Id", .{k});
            if (utils.sobject_get(&obj.fields, fk_name) != null) continue;
            if (v.sobject.id) |sid| {
                rel_keys_buf[rel_count] = fk_name;
                rel_vals_buf[rel_count] = Value{ .string = sid };
                rel_count += 1;
                continue;
            }
            if (self.resolve_external_id_reference(v.sobject)) |resolved_id| {
                rel_keys_buf[rel_count] = fk_name;
                rel_vals_buf[rel_count] = Value{ .string = resolved_id };
                rel_count += 1;
            }
        }
    ...
    /// Look for an external-Id field (`*__c` / `*Id__c`) on `ref` and find a
    /// stored record of the same type matching that value. Returns the stored
    /// record's Id when a match is found.
    fn resolve_external_id_reference(
        self: *Evaluator,
        ref: *types.SObject,
    ) ?[]const u8 {
        for (ref.fields.keys(), ref.fields.values()) |rk, rv| {
            if (!(std.mem.endsWith(u8, rk, "__c") or std.mem.endsWith(u8, rk, "Id__c"))) continue;
            if (rv != .string) continue;
            const records = self.store.get(ref.type_name) orelse return null;
            for (records.items) |rec| {
                if (rec != .sobject or rec.sobject.id == null) continue;
                const stored_val = utils.sobject_get(&rec.sobject.fields, rk) orelse continue;
                if (stored_val == .string and std.mem.eql(u8, stored_val.string, rv.string)) {
                    return rec.sobject.id.?;
                }
            }
            return null; // Only check first external-Id field.
        }
        return null;
    }

    /// Append a shallow snapshot to the store so later mutations to the live
    /// object don't affect the stored row. Auto-generated audit timestamps
    /// are written directly onto the snapshot (not the live object), so the
    /// caller observes system-set fields only via future queries.
    fn append_snapshot_to_store(
        self: *Evaluator,
        obj: *types.SObject,
        id: []const u8,
        now_str: []const u8,
        flags: InsertAuditFlags,
    ) !*types.SObject {
    ...
    fn apply_insert_type_specific_defaults(
        self: *Evaluator,
        obj: *types.SObject,
        ...
        if (std.ascii.eqlIgnoreCase(obj.type_name, "ContentVersion")) {
            ...
            try self.auto_create_content_document_for_version(obj, snapshot, id);
        }
        if (std.ascii.eqlIgnoreCase(obj.type_name, "ContentDocumentLink")) {
            try self.validate_and_attach_content_document_link(obj, snapshot);
        }
        if (std.ascii.eqlIgnoreCase(obj.type_name, "Campaign")) {
            try self.auto_create_campaign_member_statuses(id);
        }
        if (std.ascii.eqlIgnoreCase(obj.type_name, "EmailMessage")) {
            try self.auto_create_email_message_relations(obj, id);
        }
    ...
    /// Validate the LinkedEntityId references an existing record and attach a
    /// nested ContentDocument reference from the store for SOQL projections.
    fn validate_and_attach_content_document_link(
        self: *Evaluator,
        obj: *types.SObject,
        snapshot: *types.SObject,
    ) !void {
        if (utils.sobject_get(&obj.fields, "LinkedEntityId")) |lid_val| {
            if (lid_val == .string) {
                const lid = lid_val.string;
                var found_linked = false;
                var store_iter = self.store.iterator();
                while (store_iter.next()) |entry| {
                    for (entry.value_ptr.items) |rec| {
                        if (rec == .sobject and rec.sobject.id != null and
                            std.mem.eql(u8, rec.sobject.id.?, lid))
                        {
                            found_linked = true;
                            break;
                        }
                ...
                if (!found_linked) {
                    return self.throw_dml_exception(
                        "INVALID_FIELD_FOR_INSERT_UPDATE: LinkedEntityId does not " ++
                            "reference a valid record",
                    );
                }
            }
        }
        const cd_id_val = utils.sobject_get(&obj.fields, "ContentDocumentId") orelse return;
        if (cd_id_val != .string) return;
        const cd_rec = self.find_record_by_id("ContentDocument", cd_id_val.string) orelse return;
        if (cd_rec != .sobject) return;
        try obj.fields.put(self.arena, "ContentDocument", cd_rec);
        try snapshot.fields.put(self.arena, "ContentDocument", cd_rec);
    }

    /// Salesforce always creates a ContentDocument + optional ContentDocumentLink
    /// when a ContentVersion is inserted.
    fn auto_create_content_document_for_version(
        self: *Evaluator,
        obj: *types.SObject,
        snapshot: *types.SObject,
        id: []const u8,
        ...
        const path_on_client = if (utils.sobject_get(&obj.fields, "PathOnClient")) |poc|
            ...
            "";
        const file_type = content_version_file_type(path_on_client);
        ...
        if (first_pub_loc != null and first_pub_loc.? != .null_val) {
            try self.auto_create_content_document_link(cd_id, id, file_type, first_pub_loc.?);
        }
    }

    fn content_version_file_type(path_on_client: []const u8) []const u8 {
        const ew = std.ascii.endsWithIgnoreCase;
        if (ew(path_on_client, ".png")) return "PNG";
        if (ew(path_on_client, ".jpg") or ew(path_on_client, ".jpeg")) return "JPG";
        if (ew(path_on_client, ".gif")) return "GIF";
        if (ew(path_on_client, ".pdf")) return "PDF";
        if (ew(path_on_client, ".docx")) return "WORD_X";
        if (ew(path_on_client, ".xlsx")) return "EXCEL_X";
        if (ew(path_on_client, ".pptx")) return "POWER_POINT_X";
        if (ew(path_on_client, ".m4a")) return "M4A";
        return "UNKNOWN";
    }

    fn auto_create_content_document_link(
        self: *Evaluator,
        cd_id: []const u8,
        cv_id: []const u8,
        file_type: []const u8,
        ...
        try cd_ref.fields.put(self.arena, "LatestPublishedVersionId", Value{ .string = cv_id });
    ...
    fn auto_create_campaign_member_statuses(
        self: *Evaluator,
        campaign_id: []const u8,
    ) !void {
        const default_statuses = [_]struct {
            label: []const u8,
            is_default: bool,
            has_responded: bool,
            sort_order: i64,
    ...
    fn auto_create_email_message_relations(
        self: *Evaluator,
        obj: *types.SObject,
        message_id: []const u8,
    ) !void {
        ...
        for (to_ids_val.list.items.items) |to_id_item| {
            ...
            try emr.fields.put(self.arena, "EmailMessageId", Value{ .string = message_id });
    ...
    fn apply_missing_field_defaults(self: *Evaluator, obj: *types.SObject) !void {
        if (self.field_defaults.get(obj.type_name)) |defaults| {
            for (defaults.keys(), defaults.values()) |field_name, field_default| {
                if (self.get_s_object_field_value_case_insensitive(obj, field_name) != null)
                    continue;
                try utils.sobject_put(&obj.fields, self.arena, field_name, field_default);
            }
        }
    ...
    /// Auto-maintain RecordCount on DuplicateRecordSet when DuplicateRecordItem is
    /// inserted/deleted.
    ...
    /// Auto-delete DuplicateRecordSet records with RecordCount < 2 (and not freshly created with
    /// count 0).
    /// This mirrors the Salesforce trigger chain behavior: DRI delete → DRS trigger → auto-delete
    /// orphaned DRS.
            ...
            while (i < records.items.len) {
                ...
                if (rec == .sobject) {
                    const count = if (utils.sobject_get(&rec.sobject.fields, "RecordCount")) |v|
                        ...
                        0;
                    // Delete DRS when RecordCount > 0 but < 2 (i.e., 1 — single duplicate
                    // remaining)
                    // Also delete when RecordCount == 0 and there were items before (indicated by
                    // Object_Type__c being set)
    ...
    fn attach_inaccessible_fields_to_exception(
        ...
        if (from_start > select_start) {
            ...
            while (field_iter.next()) |raw_field| {
                ...
                const include = object_denied or !self.resolve_field_read_access_by_name(
                    from_type_name,
                    trimmed,
                );
                if (!include) continue;
    ...
    pub fn execute_soql(self: *Evaluator, raw: []const u8, current_env: *Env) !Value {
        ...
        if (self.is_restricted_user) try self.soql_check_restricted_security_and_throw(soql);
        ...
        if (try self.soql_try_count_query(soql, current_env, all_rows)) |cnt| return cnt;
        if (try self.soql_try_aggregate_dispatch(soql, current_env, all_rows)) |agg| return agg;

        const from_type = extract_from_type(soql) orelse return self.make_empty_list();
        var records: std.ArrayListUnmanaged(Value) = .empty;
        try self.soql_collect_store_records(soql, current_env, from_type, all_rows, &records);
        ...
        try self.soql_seed_perm_set_from_in_clause(soql, current_env, from_type, &records);
        try self.soql_load_custom_metadata_mdt(soql, current_env, from_type, all_rows, &records);
        if (all_rows) try self.soql_collect_trash_records(soql, current_env, from_type, &records);

        try self.soql_ensure_select_fields_present(soql, &records);
        try self.soql_apply_metadata_stubs(soql, current_env, from_type, &records);
        try self.soql_seed_synthetic_system_records(soql, current_env, from_type, &records);
        try self.soql_seed_fallback_user(soql, current_env, from_type, &records);
        try self.soql_seed_fallback_profile(soql, current_env, from_type, &records);
        try self.soql_seed_fallback_business_hours(soql, current_env, from_type, &records);
        try self.soql_throw_if_unknown_type(soql, current_env, from_type, &records);
        try self.soql_apply_sub_query(soql, from_type, &records);
        try self.apply_parent_field_lookups(soql, from_type, &records);
        try self.resolve_formula_fields(soql, &records);
        try self.soql_reorder_by_in_bind(soql, current_env, &records);
        self.soql_apply_order_by(soql, &records);
        self.soql_apply_offset(soql, current_env, &records);
        self.soql_apply_limit(soql, current_env, &records);
    ...
    fn soql_check_restricted_security_and_throw(
        self: *Evaluator,
        soql: []const u8,
    ) !void {
    ...
    fn soql_try_count_query(
        self: *Evaluator,
        soql: []const u8,
        current_env: *Env,
        include_all_rows: bool,
    ) !?Value {
        if (std.ascii.indexOfIgnoreCase(soql, "count()") == null) return null;
        const ft = extract_from_type(soql) orelse return Value{ .integer = 0 };
        var count: i64 = 0;
        var store_iter = self.store.iterator();
        while (store_iter.next()) |entry| {
    ...
    fn soql_try_aggregate_dispatch(
        self: *Evaluator,
        soql: []const u8,
        current_env: *Env,
        include_all_rows: bool,
        ...
        const has_agg_fn =
            std.ascii.indexOfIgnoreCase(soql, "SUM(") != null or
            std.ascii.indexOfIgnoreCase(soql, "AVG(") != null or
            std.ascii.indexOfIgnoreCase(soql, "MIN(") != null or
            std.ascii.indexOfIgnoreCase(soql, "MAX(") != null or
            std.ascii.indexOfIgnoreCase(soql, "COUNT(") != null;
        const has_count_parens = std.ascii.indexOfIgnoreCase(soql, "count()") != null;
        const has_group_by = std.ascii.indexOfIgnoreCase(soql, "group by") != null;
        if (has_agg_fn and !(has_count_parens and !has_group_by)) {
    ...
    fn soql_collect_store_records(
        ...
        var store_iter = self.store.iterator();
        while (store_iter.next()) |entry| {
            if (!std.ascii.eqlIgnoreCase(entry.key_ptr.*, from_type)) continue;
            for (entry.value_ptr.items) |record| {
    ...
    fn soql_seed_perm_set_from_in_clause(
        self: *Evaluator,
        soql: []const u8,
        current_env: *Env,
        from_type: []const u8,
        ...
        if (!(std.ascii.eqlIgnoreCase(from_type, "PermissionSet") or
            std.ascii.eqlIgnoreCase(from_type, "PermissionSetLicense"))) return;
        const wc = extract_where_clause(soql) orelse return;
        if (std.ascii.indexOfIgnoreCase(wc, " IN :") == null and
            std.ascii.indexOfIgnoreCase(wc, " IN (") == null) return;
        try self.seed_named_records(from_type, soql, current_env, records);
    }

    fn soql_load_custom_metadata_mdt(
        self: *Evaluator,
        soql: []const u8,
        current_env: *Env,
        from_type: []const u8,
        ...
        if (records.items.len != 0) return;
        if (!std.mem.endsWith(u8, from_type, "__mdt")) return;
        if (self.store.get(from_type) != null) return;
    ...
    fn soql_ensure_select_fields_present(
        self: *Evaluator,
        soql: []const u8,
        records: *std.ArrayListUnmanaged(Value),
    ) !void {
        ...
        if (from_start2 <= select_start2) return;
        const select_clause2 = std.mem.trim(u8, soql[select_start2..from_start2], " \t\n\r");
        if (std.ascii.indexOfIgnoreCase(select_clause2, "FIELDS(") != null) return;
        if (std.ascii.indexOfIgnoreCase(select_clause2, "COUNT(") != null) return;
        if (std.ascii.indexOfIgnoreCase(select_clause2, "SUM(") != null) return;
        var field_iter = std.mem.splitScalar(u8, select_clause2, ',');
        while (field_iter.next()) |raw_field| {
            try self.soql_apply_select_field_to_records(raw_field, records);
        }
    }

    fn soql_apply_select_field_to_records(
        self: *Evaluator,
        raw_field: []const u8,
        records: *std.ArrayListUnmanaged(Value),
    ) !void {
        ...
        if (field_name[0] == '(') return; // subquery
        var is_to_label = false;
        if (std.ascii.startsWithIgnoreCase(field_name, "toLabel(") and
            std.mem.endsWith(u8, field_name, ")"))
        {
        ...
        for (records.items) |item| {
            ...
            const fv = utils.sobject_get(&item.sobject.fields, field_name) orelse continue;
            if (fv != .string) continue;
            const label = self.resolve_picklist_label(
                item.sobject.type_name,
                field_name,
                fv.string,
            ) orelse continue;
    ...
    fn soql_apply_metadata_stubs(
        ...
        if (std.ascii.eqlIgnoreCase(from_type, "FieldPermissions")) {
            try self.soql_seed_field_permissions(soql, current_env, records);
            return;
        }
        const is_static_or_apex =
            std.ascii.eqlIgnoreCase(from_type, "StaticResource") or
            std.ascii.eqlIgnoreCase(from_type, "ApexClass");
        if (is_static_or_apex and std.ascii.indexOfIgnoreCase(soql, " IN (") != null) {
            try self.soql_seed_metadata_in_clause(soql, current_env, from_type, records);
            return;
        }
        if (std.ascii.eqlIgnoreCase(from_type, "ApexClass") and
            std.ascii.indexOfIgnoreCase(soql, " OR ") != null)
        {
            try self.soql_seed_apex_class_or(soql, current_env, from_type, records);
            return;
        }
    ...
    fn soql_seed_field_permissions(
        self: *Evaluator,
        soql: []const u8,
        current_env: *Env,
        records: *std.ArrayListUnmanaged(Value),
    ) !void {
        const obj_type = self.extract_where_field_value(
            soql,
            "SobjectType",
            current_env,
        ) orelse "Account";
    ...
    fn soql_seed_metadata_in_clause(
        self: *Evaluator,
        soql: []const u8,
        current_env: *Env,
        from_type: []const u8,
    ...
    fn soql_seed_apex_class_or(
        ...
        while (wpos < where_clause.len) {
            const name_pos = std.ascii.indexOfIgnoreCasePos(
                where_clause,
                wpos,
                "Name",
            ) orelse break;
            const extracted = self.soql_extract_apex_class_or_name(
                where_clause,
                name_pos,
                current_env,
            ) orelse {
            ...
            wpos = extracted.next_pos;
            const nv = extracted.name orelse continue;
            const tmp_soql = try std.fmt.allocPrint(
                self.arena,
    ...
    const ApexClassOrName = struct { name: ?[]const u8, next_pos: usize };

    fn soql_extract_apex_class_or_name(
        self: *Evaluator,
        where_clause: []const u8,
        name_pos: usize,
        current_env: *Env,
        ...
        if (j < where_clause.len and where_clause[j] == '\'') {
            ...
            if (std.mem.indexOfPos(u8, where_clause, start, "'")) |end| {
                return .{ .name = where_clause[start..end], .next_pos = end + 1 };
            }
            return .{ .name = null, .next_pos = j + 1 };
        }
        if (j < where_clause.len and where_clause[j] == ':') {
            const start = j + 1;
            var end = start;
            while (end < where_clause.len and
                (std.ascii.isAlphanumeric(where_clause[end]) or where_clause[end] == '_')) end += 1;
            ...
            return .{ .name = name_val, .next_pos = end };
        }
        return null;
    }

    fn soql_seed_synthetic_system_records(
        self: *Evaluator,
        soql: []const u8,
        current_env: *Env,
        from_type: []const u8,
        ...
        if (std.ascii.eqlIgnoreCase(from_type, "User")) {
            try self.soql_seed_user_synthetic(soql, current_env, records);
        } else if (std.ascii.eqlIgnoreCase(from_type, "Profile")) {
            try self.soql_seed_profile_synthetic(soql, current_env, records);
        } else if (std.ascii.eqlIgnoreCase(from_type, "RecordType")) {
            try self.soql_seed_record_type_synthetic(soql, current_env, records);
        }
    }

    fn soql_seed_user_synthetic(
        self: *Evaluator,
        soql: []const u8,
        current_env: *Env,
        records: *std.ArrayListUnmanaged(Value),
    ) !void {
        const use_query_specific_user =
            self.has_exact_where_field_comparison(soql, "Profile.Name") or
        ...
        const user_record = if (self.query_matches_current_user(soql, current_env))
            ...
            try self.create_current_user_record();
        if (user_record == .null_val) return;
        if (!self.matches_where(user_record, soql, current_env)) return;
    ...
    fn soql_seed_profile_synthetic(
        self: *Evaluator,
        soql: []const u8,
        current_env: *Env,
        records: *std.ArrayListUnmanaged(Value),
        ...
        if (!self.matches_where(profile_record, soql, current_env)) return;
    ...
    fn soql_seed_record_type_synthetic(
        self: *Evaluator,
        soql: []const u8,
        current_env: *Env,
        records: *std.ArrayListUnmanaged(Value),
        ...
        const rt_records = self.store.get("RecordType") orelse return;
    ...
    fn soql_seed_fallback_user(
        ...
        if (records.items.len != 0) return;
        if (!std.ascii.eqlIgnoreCase(from_type, "User")) return;
        if (!self.should_synthesize_builtin_user(soql, current_env)) return;
        const user_record = if (self.query_matches_current_user(soql, current_env))
            try self.create_current_user_record()
    ...
    fn soql_seed_fallback_profile(
        ...
        if (records.items.len != 0) return;
        if (!std.ascii.eqlIgnoreCase(from_type, "Profile")) return;
    ...
    fn soql_seed_fallback_business_hours(
        self: *Evaluator,
        soql: []const u8,
        current_env: *Env,
        from_type: []const u8,
        ...
        if (records.items.len != 0) return;
        if (!std.ascii.eqlIgnoreCase(from_type, "BusinessHours")) return;
        ...
        if (!self.matches_where(Value{ .sobject = business_hours }, soql, current_env)) return;
        try records.append(self.arena, Value{ .sobject = business_hours });
    }

    fn soql_is_known_sobject_type(from_type: []const u8) bool {
        const known_types = [_][]const u8{
            ...
            "PermissionSetGroup",         "PlatformCachePartition", "Metadata_Driven_Trigger__mdt",
            "CronTrigger",                "AsyncApexJob",           "EntityDefinition",
            "FieldDefinition",            "AggregateResult",        "RecordType",
            "DuplicateRule",              "DuplicateRecordSet",     "DuplicateRecordItem",
            "UserRecordAccess",           "AuthSession",            "LoginHistory",
            "TaskStatus",                 "BusinessHours",          "FeedItem",
            "CollaborationGroup",         "UserRole",               "GroupMember",
            "Group",                      "Attachment",             "Note",
            "EmailMessage",               "CaseComment",            "Solution",
            "Contract",                   "Product2",               "Pricebook2",
            "PricebookEntry",             "OpportunityLineItem",    "Quote",
            "QuoteLineItem",              "PermissionSetLicense",   "EmailTemplate",
            "Folder",                     "Document",               "CampaignMember",
            "CampaignMemberStatus",       "EmailMessageRelation",   "OrgWideEmailAddress",
            "PermissionSetLicenseAssign", "ServiceResource",        "AssignedResource",
            "ServiceTerritory",           "ServiceTerritoryMember", "ApexTrigger",
            "CustomPermission",           "FlowDefinitionView",     "FlowVersionView",
            "ApexEmailNotification",      "Network",                "Topic",
            "OmniProcess",                "AppMenuItem",            "LeadStatus",
            "UserPreference",             "UserLogin",              "LoginIp",
            "LeadHistory",                "AccountHistory",         "EventBusSubscriber",
            "BatchApexErrorEvent",        "AsyncOperationEvent",    "AsyncOperationStatus",
        };
        for (known_types) |kt| {
            if (std.ascii.eqlIgnoreCase(from_type, kt)) return true;
        }
        return std.mem.endsWith(u8, from_type, "__c") or
    ...
    fn soql_throw_if_unknown_type(
        ...
        if (records.items.len != 0) return;
        if (self.store.get(from_type) != null) return;
        if (soql_is_known_sobject_type(from_type)) return;
    ...
    fn soql_apply_sub_query(
        ...
        for (records.items) |*rec| {
            ...
            if (child_type) |ct| {
                try self.soql_collect_sub_query_children(
                    ct,
                    from_type,
                    rel_name,
                    parent_id,
    ...
    fn soql_collect_sub_query_children(
        self: *Evaluator,
        child_type: []const u8,
        from_type: []const u8,
        rel_name: []const u8,
        parent_id: []const u8,
        child_records: *std.ArrayListUnmanaged(Value),
    ) !void {
        var child_iter = self.store.iterator();
        while (child_iter.next()) |child_entry| {
            if (!std.ascii.eqlIgnoreCase(child_entry.key_ptr.*, child_type)) continue;
            const fk_field = self.resolve_foreign_key(child_type, from_type, rel_name);
            for (child_entry.value_ptr.items) |child_rec| {
                if (child_rec != .sobject) continue;
                const fk_val = utils.sobject_get(
                    &child_rec.sobject.fields,
                    fk_field,
                ) orelse continue;
                if (fk_val != .string) continue;
                if (!std.ascii.eqlIgnoreCase(fk_val.string, parent_id)) continue;
    ...
    fn soql_reorder_by_in_bind(
        ...
        const bind_info = self.extract_where_in_bind_info(soql) orelse return;
        const bind_val = self.lookup_bind_value(current_env, bind_info.bind_name) orelse return;
        try self.reorder_records_by_in_bind(records, bind_info.field_name, bind_val);
    }

    fn soql_apply_order_by(
        self: *Evaluator,
        soql: []const u8,
        records: *std.ArrayListUnmanaged(Value),
    ) void {
    ...
    fn soql_apply_offset(
        ...
        const offset_val = offset_val_opt orelse return;
        if (offset_val < records.items.len) {
            const remaining = records.items.len - offset_val;
            std.mem.copyForwards(
                Value,
                records.items[0..remaining],
                records.items[offset_val..records.items.len],
            );
    ...
    fn soql_apply_limit(
        ...
        const limit_val = limit_val_opt orelse return;
    ...
    fn generate_metadata_stub(
        ...
        if (std.ascii.eqlIgnoreCase(from_type, "Profile")) {
            return try self.generate_profile_stub(soql, current_env);
        }
        if (std.ascii.eqlIgnoreCase(from_type, "ContentVersion")) {
            return try self.generate_content_version_stub(soql);
        }
        if (std.ascii.eqlIgnoreCase(from_type, "CronTrigger")) {
            return try self.generate_cron_trigger_stub(soql, current_env);
        }
        if (std.ascii.eqlIgnoreCase(from_type, "Organization")) {
            return try self.generate_organization_stub();
        }
        if (std.ascii.eqlIgnoreCase(from_type, "AppMenuItem")) {
            return try self.generate_app_menu_item_stub();
        }
        if (std.ascii.eqlIgnoreCase(from_type, "PlatformCachePartition")) {
            return try self.generate_platform_cache_partition_stub();
        }
        if (std.ascii.eqlIgnoreCase(from_type, "DuplicateRule")) {
            return try self.generate_duplicate_rule_stub(name_val);
        }
        if (std.ascii.eqlIgnoreCase(from_type, "StaticResource")) {
            return try self.generate_static_resource_stub(name_val);
        }
        if (std.ascii.eqlIgnoreCase(from_type, "Metadata_Driven_Trigger__mdt")) return null;
        if (std.ascii.eqlIgnoreCase(from_type, "Bucketed_Picklist__mdt")) {
            return try self.generate_bucketed_picklist_stub();
        }
        if (std.ascii.eqlIgnoreCase(from_type, "Picklist_Bucket__mdt")) {
            return try self.generate_picklist_bucket_stub();
        }
        return null;
    }

    fn generate_custom_permission_stub(
        self: *Evaluator,
        from_type: []const u8,
        soql: []const u8,
        current_env: *Env,
        ...
        const developer_name = self.extract_where_field_value(
            soql,
            "DeveloperName",
            current_env,
        ) orelse name_val;
        if (self.store.get(from_type)) |records| {
            for (records.items) |record| {
                if (record != .sobject) continue;
                const stored = utils.sobject_get(
                    &record.sobject.fields,
                    "DeveloperName",
                ) orelse continue;
                if (stored == .string and
                    std.ascii.eqlIgnoreCase(stored.string, developer_name)) return record;
            }
        }
        const sob = try self.arena.create(types.SObject);
    ...
    fn generate_permission_set_group_stub(
        self: *Evaluator,
        name_val: []const u8,
    ) !Value {
        const sob = try self.arena.create(types.SObject);
    ...
    fn generate_profile_stub(
        ...
        const profile_record = if (use_query_specific_profile)
            ...
            try self.create_current_profile_record();
        if (!self.matches_where(profile_record, soql, current_env)) return null;
        if (profile_record == .sobject) {
            // Persist so isRestrictedUser() can look it up in later calls.
            const gop = try self.store.getOrPut(self.arena, "Profile");
            if (!gop.found_existing) gop.value_ptr.* = .empty;
            try gop.value_ptr.append(self.arena, profile_record);
        }
    ...
    /// Only stub ContentVersion when no WHERE clause filters by field; a
    /// WHERE-bound query must return empty if the record doesn't exist so the
    /// caller can raise QueryException on single-record assignments.
    fn generate_content_version_stub(self: *Evaluator, soql: []const u8) !?Value {
        if (extract_where_clause(soql) != null) return null;
        const sob = try self.arena.create(types.SObject);
    ...
    fn generate_cron_trigger_stub(
        ...
        if (std.mem.indexOf(u8, where_clause, ":")) |bind_pos| {
            ...
            while (end_pos < rest.len and
                (std.ascii.isAlphanumeric(rest[end_pos]) or
                    rest[end_pos] == '_')) : (end_pos += 1)
            {}
            if (end_pos > 0) {
    ...
    fn generate_organization_stub(self: *Evaluator) !Value {
        const sob = try self.arena.create(types.SObject);
        ...
        const created_by_id = if (created_by == .sobject and created_by.sobject.id != null)
            ...
            self.current_user_id;
        const id = "00D000000000001"; // Singleton Id.
        sob.id = id;
        try sob.fields.put(self.arena, "Id", Value{ .string = id });
    ...
    fn generate_app_menu_item_stub(self: *Evaluator) !Value {
    ...
    fn generate_platform_cache_partition_stub(self: *Evaluator) !Value {
        const sob = try self.arena.create(types.SObject);
    ...
    fn generate_duplicate_rule_stub(self: *Evaluator, name_val: []const u8) !Value {
        const sob = try self.arena.create(types.SObject);
    ...
    /// StaticResource stub: returns a single record (IN clause with multiple
    /// names is the caller's responsibility to expand).
    fn generate_static_resource_stub(self: *Evaluator, name_val: []const u8) !Value {
        const sob = try self.arena.create(types.SObject);
    ...
    fn generate_bucketed_picklist_stub(self: *Evaluator) !Value {
        const sob = try self.arena.create(types.SObject);
    ...
    fn generate_picklist_bucket_stub(self: *Evaluator) !Value {
        const sob = try self.arena.create(types.SObject);
    ...
    fn scan_bind_expression_end(source: []const u8, start: usize) usize {
        var j = start;
    ...
    fn lookup_evaluated_bind_expression(
        ...
        const expr = parser_mod.parse_expr(tokens, self.arena) catch return null;
    ...
    fn lookup_bind_value(self: *Evaluator, current_env: *Env, var_name: []const u8) ?Value {
        ...
        if (self.lookup_evaluated_bind_expression(current_env, var_name)) |bv| return bv;
    ...
    fn extract_where_name_value(self: *Evaluator, soql: []const u8, current_env: *Env) ?[]const u8 {
    ...
    fn has_where_field_null_literal(
        ...
        while (pos + field_name.len <= clause.len) : (pos += 1) {
            ...
            j += 1;
            while (j < clause.len and is_soql_whitespace(clause[j])) j += 1;
            if (j + 4 <= clause.len and std.ascii.eqlIgnoreCase(clause[j .. j + 4], "NULL"))
                return true;
        }
        return false;
    }
    ...
    fn alloc_id(self: *Evaluator) ![]const u8 {
    ...
    fn load_static_resource_body(self: *Evaluator, name: []const u8) ?[]const u8 {
        ...
        for (self.source_paths) |sp| {
            ...
            if (self.find_static_resource_in_dir(sp, name, &extensions)) |content| return content;
        }
        return null;
    }
    ...
    fn decode_custom_metadata_value(val_tag: []const u8, field_value: []const u8) Value {
        ...
        {
            return if (std.fmt.parseFloat(
                f64,
                field_value,
            )) |f| Value{ .double = f } else |_| Value{ .string = field_value };
        }
        if (std.mem.indexOf(u8, val_tag, "xsd:int") != null) {
            return if (std.fmt.parseInt(
                i64,
                field_value,
                10,
            )) |i| Value{ .integer = i } else |_| Value{ .string = field_value };
        }
    ...
    fn xml_tag_bool_value(self: *Evaluator, xml: []const u8, tag_name: []const u8) ?bool {
    ...
    fn append_store_record(self: *Evaluator, type_name: []const u8, record: *types.SObject) !void {
    ...
    fn find_permission_set_record_by_name(
        ...
        for (permission_sets.items) |item| {
            if (item != .sobject) continue;
            const name_val = utils.sobject_get(&item.sobject.fields, "Name") orelse continue;
    ...
    fn append_expanded_permission_fields(
        ...
        const components = composite_permission_field_components(field_name) orelse return;
    ...
    fn load_permission_set_metadata_from_files(self: *Evaluator, requested_name: []const u8) !void {
        if (self.find_permission_set_record_by_name(requested_name) != null) return;
        ...
        for (self.source_paths) |sp| {
            ...
            while (walker.next(self.io) catch null) |entry| {
                if (!permission_set_entry_matches(entry, requested_name)) continue;
                ...
                const permission_set_id = try self.create_permission_set_record(
                    permission_set_name,
                    xml,
                );
                try self.load_object_permissions(xml, permission_set_id);
                try self.load_field_permissions(xml, permission_set_id);
                return;
            }
        }
    }

    fn permission_set_entry_matches(entry: anytype, requested_name: []const u8) bool {
        if (entry.kind != .file) return false;
        if (std.mem.indexOf(u8, entry.path, "permissionsets/") == null) return false;
        if (!std.mem.endsWith(u8, entry.path, ".permissionset-meta.xml")) return false;
        const suffix = ".permissionset-meta.xml";
        const file_name = std.fs.path.basename(entry.path);
        if (file_name.len <= suffix.len) return false;
        const permission_set_name = file_name[0 .. file_name.len - suffix.len];
        return std.ascii.eqlIgnoreCase(permission_set_name, requested_name);
    }

    fn create_permission_set_record(
        self: *Evaluator,
        permission_set_name: []const u8,
        xml: []const u8,
    ) ![]const u8 {
        ...
        try permission_set.fields.put(
            self.arena,
            "Label",
            Value{ .string = try self.arena.dupe(
                u8,
    ...
    fn load_object_permissions(
        ...
        while (object_pos < xml.len) {
            const block_start = std.mem.indexOfPos(
                u8,
                xml,
                object_pos,
                "<objectPermissions>",
            ...
            if (self.xml_tag_value(block, "object")) |object_name| {
                try self.append_object_permission(block, object_name, permission_set_id);
            }
            object_pos = block_end + "</objectPermissions>".len;
        }
    }

    fn append_object_permission(
        self: *Evaluator,
        block: []const u8,
        ...
        object_permission.* = .{
            .type_name = "ObjectPermissions",
            .id = try self.alloc_id(),
        };
        ...
        try object_permission.fields.put(self.arena, "PermissionsRead", Value{
            .boolean = self.xml_tag_bool_value(block, "allowRead") orelse false,
        });
        try object_permission.fields.put(self.arena, "PermissionsCreate", Value{
            .boolean = self.xml_tag_bool_value(block, "allowCreate") orelse false,
        });
        try object_permission.fields.put(self.arena, "PermissionsEdit", Value{
            .boolean = self.xml_tag_bool_value(block, "allowEdit") orelse false,
        });
        try object_permission.fields.put(self.arena, "PermissionsDelete", Value{
    ...
    fn load_field_permissions(
        ...
        while (field_pos < xml.len) {
            ...
            const block = xml[block_content_start..block_end];
            if (self.xml_tag_value(block, "field")) |full_field_name| {
                try self.append_expanded_field_permissions(
                    block,
                    full_field_name,
                    permission_set_id,
                );
    ...
    fn append_expanded_field_permissions(
        self: *Evaluator,
        block: []const u8,
        full_field_name: []const u8,
        permission_set_id: []const u8,
        ...
        for (expanded_fields.items) |expanded_field_name| {
            ...
            try field_permission.fields.put(self.arena, "PermissionsRead", Value{
                .boolean = self.xml_tag_bool_value(block, "readable") orelse false,
            });
            try field_permission.fields.put(self.arena, "PermissionsCreate", Value{
                .boolean = self.xml_tag_bool_value(block, "editable") orelse false,
            });
            try field_permission.fields.put(self.arena, "PermissionsEdit", Value{
                .boolean = self.xml_tag_bool_value(block, "editable") orelse false,
            });
    ...
    fn extract_where_in_bind_info(
        self: *Evaluator,
        soql: []const u8,
    ) ?struct { field_name: []const u8, bind_name: []const u8 } {
        _ = self;
        const where_clause = extract_where_clause(soql) orelse return null;
    ...
    fn bind_collection_contains(self: *Evaluator, field_val: Value, bind_val: Value) bool {
        switch (bind_val) {
            .list => |list| {
                for (list.items.items) |item| {
                    if (utils.value_eql(field_val, item)) return true;
    ...
    fn eval_simple_condition(
        ...
        const field_val = field_val_opt orelse {
            return self.simple_condition_missing_field(
                cond,
                field_name,
                value_str,
                op,
        ...
        const cmp_val = self.resolve_simple_condition_cmp_value(
            value_str,
            op.is_neq,
            current_env,
        ) orelse
    ...
    fn hydrate_parent_chain(
        ...
        while (std.mem.indexOfScalar(u8, remaining, '.')) |dot_pos| {
            ...
            const parent_type = self.parent_ref_to_type_for_s_object(
                current_type,
                parent_ref,
            ) orelse
                return;
    ...
    fn resolve_formula_fields(
        ...
        while (iter.next()) |field_part| {
            ...
            if (trimmed.len > 0 and trimmed[0] == '(') continue;
            // Pattern: <Prefix>_<Suffix>__c where <Prefix>__c is a FK and <Suffix> is a parent
            // field
    ...
    pub fn get_s_object_field_value_case_insensitive(
        ...
        if (self.resolve_custom_child_relationship(sob.type_name, field_name) != null or
            (field_name.len > 3 and std.ascii.eqlIgnoreCase(
    ...
    fn resolve_field_path_value(
        ...
        while (true) {
            const dot_pos = std.mem.indexOfScalar(u8, remaining, '.') orelse {
                return self.get_s_object_field_value_case_insensitive(current, remaining);
            };
    ...
    fn collect_summary_impact_ids_from_records(
        ...
        for (records) |record| {
            if (record != .sobject) continue;
            const fk_val = self.get_s_object_field_value_case_insensitive(
                record.sobject,
                fk_field,
            ) orelse continue;
    ...
    fn compute_formula_field_value(
        ...
        if (std.mem.indexOfScalar(u8, trimmed, '+')) |_| {
            ...
            while (term_iter.next()) |raw_term| {
                ...
                const term_value = self.get_s_object_field_value_case_insensitive(
                    sob,
                    term,
                ) orelse Value.null_val;
                switch (term_value) {
        ...
        if (std.ascii.indexOfIgnoreCase(trimmed, "!= null")) |idx| {
            ...
            const lhs_value = self.get_s_object_field_value_case_insensitive(
                sob,
                lhs,
            ) orelse Value.null_val;
    ...
    fn resolve_instance_field_live(self: *Evaluator, name: []const u8, current_env: *Env) ?Value {
        ...
        if (!(self.is_instance_field(
            live_cd,
            name,
        ) or self.is_parent_instance_field(live_cd, name))) {
            return null;
        }
    ...
    fn resolve_this_getter_or_backing(
        self: *Evaluator,
        name: []const u8,
        current_env: *Env,
    ) !?Value {
    ...
    fn resolve_class_static_field_from_this(
        self: *Evaluator,
        name: []const u8,
        current_env: *Env,
    ) ?Value {
    ...
    fn try_schema_prefix_field_access(
        self: *Evaluator,
        schema_name: []const u8,
        field: []const u8,
    ) !?Value {
        if (is_known_schema_enum_type(schema_name)) return Value{ .string = field };
        if (std.ascii.eqlIgnoreCase(schema_name, "SObjectType")) {
            const sot = try self.arena.create(types.ObjectInstance);
            sot.* = .{ .class_name = "Schema.SObjectType" };
            try sot.fields.put(self.arena, "name", Value{ .string = field });
            return Value{ .object = sot };
        }
        if (std.ascii.eqlIgnoreCase(field, "SObjectType")) {
            const sot = try self.arena.create(types.ObjectInstance);
            sot.* = .{ .class_name = "Schema.SObjectType" };
            try sot.fields.put(self.arena, "name", Value{ .string = schema_name });
            return Value{ .object = sot };
        }
        if (self.is_s_object_type_name(schema_name) and !std.ascii.eqlIgnoreCase(field, "class")) {
            return try self.make_s_object_field_token(schema_name, field);
        }
        return null;
    }

    fn try_three_level_enum_constant(
        self: *Evaluator,
        outer_name: []const u8,
        inner_name: []const u8,
        field: []const u8,
    ) !?Value {
        self.ensure_static_init(outer_name);
        const inner_fq = try std.fmt.allocPrint(self.arena, "{s}.{s}", .{ outer_name, inner_name });
        self.ensure_static_init(inner_fq);
        const fq_key = try std.fmt.allocPrint(
            self.arena,
            "{s}.{s}.{s}",
            .{ outer_name, inner_name, field },
        );
        if (self.global_env.get(fq_key)) |v| return v;
        if (self.find_class(outer_name)) |cd| {
            for (cd.members) |member| {
                if (member == .enum_decl and
                    std.ascii.eqlIgnoreCase(member.enum_decl.name, inner_name))
                {
        ...
        if (std.ascii.eqlIgnoreCase(outer_name, "Schema") and
            ...
            !std.ascii.eqlIgnoreCase(inner_name, "SObjectType") and
            self.is_s_object_type_name(inner_name))
        {
            if (std.ascii.eqlIgnoreCase(field, "SObjectType")) {
                const sot = try self.arena.create(types.ObjectInstance);
                sot.* = .{ .class_name = "Schema.SObjectType" };
            ...
            if (!std.ascii.eqlIgnoreCase(field, "class")) {
                return try self.make_s_object_field_token(inner_name, field);
            }
        }
        return null;
    }
    ...
    fn eval_field_access_expr(
            ...
            std.mem.startsWith(u8, fa.object.identifier.name, "Schema."))
        {
            ...
            if (try self.try_schema_prefix_field_access(schema_name, fa.field)) |v| return v;
        }
        if (fa.object.* == .field_access) {
            ...
            if (inner_fa.object.* == .identifier) {
                ...
                if (try self.try_three_level_enum_constant(outer_name, inner_name, fa.field)) |v| {
                    return v;
                }
            }
        }
        if (fa.null_safe) {
            const obj = self.eval_expr(fa.object, current_env) catch |err| {
            ...
            return self.eval_field_access(fa, obj, current_env);
        }
    ...
    fn instanceof_list(self: *Evaluator, val: Value, tn: anytype) bool {
        ...
        for (val.list.items.items) |item| {
            ...
            if (item == .object and
                self.is_subclass_of(item.object.class_name, elem_type)) return true;
            return false;
        }
        return true;
    }
    ...
    fn instanceof_set_or_map(val: Value, tn_name: []const u8) bool {
        ...
        const tn_base = if (std.mem.lastIndexOfScalar(u8, tn_name, '.')) |di|
            ...
            tn_name;
        return std.ascii.eqlIgnoreCase(tn_name, "Set") or std.ascii.eqlIgnoreCase(
            tn_base,
            "Iterable",
        );
    }
    ...
    fn eval_instanceof_expr(
        ...
        return switch (val) {
            ...
            .integer, .long, .double => Value{ .boolean = instanceof_matches_primitive(
                val,
                type_name,
            ) },
    ...
    fn eval_call_expr(self: *Evaluator, call: *ast.CallExpr, current_env: *Env) anyerror!Value {
        ...
        if (try self.try_call_as_instance_method(
            call.callee,
            args.items,
            current_env,
        )) |v| return v;
    ...
    fn is_collection_type_name(type_name: []const u8) bool {
        const base = type_base_name(strip_type_namespace(type_name));
    ...
    fn strip_type_namespace(type_name: []const u8) []const u8 {
    ...
    fn extract_collection_element_type_name(type_name: []const u8) ?[]const u8 {
        const stripped = strip_type_namespace(type_name);
    ...
    fn extract_map_type_args(type_name: []const u8) ?struct { key: []const u8, value: []const u8 } {
        const stripped = strip_type_namespace(type_name);
    ...
    fn annotate_declared_collection_type(
        self: *Evaluator,
        value: Value,
        ...
        if (declared_type) |type_name| {
            if (value == .list) {
                if (value.list.element_type == null) {
                    if (extract_collection_element_type_name(type_name)) |elem_type| {
                        value.list.element_type = strip_type_namespace(elem_type);
                    }
                }
            }
        }
    ...
    fn infer_list_element_type(
        ...
        return switch (list.items.items[0]) {
            .sobject => |sob| strip_type_namespace(sob.type_name),
            .object => |obj| strip_type_namespace(obj.class_name),
            .string => |s| if (Evaluator.is_salesforce_id_string(s)) "Id" else null,
            else => null,
        };
    }

    fn score_list_argument_for_param(
        self: *Evaluator,
        list: *types.ListValue,
        arg_hint: ?[]const u8,
        param_type: types.TypeRef,
    ) i32 {
        const param_base = type_base_name(param_type.name);
        if (!std.ascii.eqlIgnoreCase(param_base, "List") and
            !std.ascii.eqlIgnoreCase(param_base, "Iterable"))
        {
            return overload_score_for_arg(Value{ .list = list }, param_type.name);
        }
        if (param_type.params.len == 0) return 2;

        const param_elem_type = strip_type_namespace(self.render_type_ref(param_type.params[0]));
        const param_elem_base = type_base_name(param_elem_type);
        const actual_elem_type = self.infer_list_element_type(list, arg_hint);
        ...
        const actual_elem_base = type_base_name(actual_elem_type.?);
        if (std.ascii.eqlIgnoreCase(param_elem_base, "SObject")) {
            if (actual_items_are_sobjects or
                ...
                self.is_s_object_type_name(actual_elem_base))
            {
                return 3;
            }
            return 0;
        }
        if (names_match_by_simple_name(actual_elem_base, param_elem_base)) return 3;
        if (self.is_s_object_type_name(actual_elem_base) and
            self.is_s_object_type_name(param_elem_base) and
            self.is_subclass_of(actual_elem_base, param_elem_base))
        {
            return 2;
        }
        return 0;
    ...
    fn type_base_name(type_name: []const u8) []const u8 {
        const stripped = strip_type_namespace(type_name);
        if (std.mem.indexOfScalar(u8, stripped, '<')) |lt| return stripped[0..lt];
        return stripped;
    }

    fn find_declared_field_type(
        self: *Evaluator,
        class_name: []const u8,
        field_name: []const u8,
    ) ?[]const u8 {
                    ...
                    switch (member) {
                        .field_decl => |fd| {
                            if (std.ascii.eqlIgnoreCase(fd.name, field_name))
                                return self.render_type_ref(fd.type_ref);
                        },
                        else => {},
                    }
                }
                cur = if (ccd.super_class) |sc| self.find_class(sc.name) else null;
            }
        }
        return null;
    }
    ...
    fn resolve_assignment_target_type(
        ...
        switch (target.*) {
            .identifier => |id| {
                ...
                if (self.current_class) |cc| {
                    if (self.find_declared_field_type(cc, id.name)) |type_name| return type_name;
            ...
            .field_access => |fa| {
                ...
                if (fa.object.* == .identifier) {
                    ...
                    if (current_env.get_declared_type(owner_name)) |owner_type| {
                        if (self.find_declared_field_type(
                            type_base_name(owner_type),
                            fa.field,
                        )) |type_name| return type_name;
                    }
                    const is_class = self.find_class(owner_name) != null;
                ...
                if (self.resolve_assignment_target_type(fa.object, current_env)) |owner_type| {
                    if (self.find_declared_field_type(
                        type_base_name(owner_type),
                        fa.field,
                    )) |type_name| return type_name;
    ...
    fn maybe_coerce_schema_expr_value(
        ...
        return switch (expr.*) {
            .identifier => |id| try self.coerce_schema_identifier(id.name, value),
            .field_access => |fa| try self.coerce_schema_field_access(fa, value),
            else => value,
        };
    }

    /// Identifier of the form `Schema.Owner.Member` where the real lookup
    /// should produce a Schema-enum value, an SObjectType token, or an
    /// SObjectField token.
    fn coerce_schema_identifier(
        self: *Evaluator,
        name: []const u8,
        value: Value,
    ) !Value {
        if (!std.mem.startsWith(u8, name, "Schema.")) return value;
        const suffix = name["Schema.".len..];
        const dot_idx = std.mem.lastIndexOfScalar(u8, suffix, '.') orelse return value;
        const owner_name = suffix[0..dot_idx];
        const member_name = suffix[dot_idx + 1 ..];
        if (is_known_schema_enum_type(owner_name)) return Value{ .string = member_name };
        if (std.ascii.eqlIgnoreCase(owner_name, "SObjectType")) {
            return try self.make_sobject_type_token(member_name);
        }
        if (std.ascii.eqlIgnoreCase(member_name, "SObjectType")) {
            return try self.make_sobject_type_token(owner_name);
        }
        if (std.ascii.eqlIgnoreCase(member_name, "class")) return value;
        return try self.make_s_object_field_token(owner_name, member_name);
    }

    /// FieldAccess chains rooted at `Schema.*` → produce the matching proxy
    /// value (enum string / SObjectType token / SObjectField token). Returns
    /// the original `value` when no Schema-based rewrite applies.
    fn coerce_schema_field_access(
        self: *Evaluator,
        fa: *ast.FieldAccess,
        value: Value,
    ) !Value {
        if (fa.object.* == .field_access) {
            if (try self.coerce_schema_two_level(fa)) |v| return v;
        }
        if (fa.object.* == .identifier and
            std.mem.startsWith(u8, fa.object.identifier.name, "Schema."))
        {
            if (try self.coerce_schema_one_level(
                fa.object.identifier.name["Schema.".len..],
                fa.field,
            )) |v| return v;
        }
        return value;
    ...
    fn coerce_schema_two_level(
        self: *Evaluator,
        fa: *ast.FieldAccess,
    ) !?Value {
        const inner_fa = fa.object.field_access;
        if (inner_fa.object.* != .identifier) return null;
        if (!std.ascii.eqlIgnoreCase(inner_fa.object.identifier.name, "Schema")) return null;
        if (is_known_schema_enum_type(inner_fa.field)) return Value{ .string = fa.field };
        if (std.ascii.eqlIgnoreCase(inner_fa.field, "sObjectType") or
            std.ascii.eqlIgnoreCase(inner_fa.field, "SObjectType"))
        {
            return try self.make_sobject_type_token(fa.field);
        }
        if (std.ascii.eqlIgnoreCase(fa.field, "SObjectType")) {
            return try self.make_sobject_type_token(inner_fa.field);
        }
    ...
    fn coerce_schema_one_level(
        self: *Evaluator,
        schema_name: []const u8,
        field_name: []const u8,
    ) !?Value {
        if (is_known_schema_enum_type(schema_name)) return Value{ .string = field_name };
        if (std.ascii.eqlIgnoreCase(schema_name, "SObjectType")) {
            return try self.make_sobject_type_token(field_name);
        }
        if (std.ascii.eqlIgnoreCase(field_name, "SObjectType")) {
            return try self.make_sobject_type_token(schema_name);
        }
    ...
    fn render_type_ref(self: *Evaluator, type_ref: types.TypeRef) []const u8 {
        if (type_ref.params.len == 0) return strip_type_namespace(type_ref.name);

        var buf: std.ArrayListUnmanaged(u8) = .empty;
        buf.appendSlice(
            self.arena,
            strip_type_namespace(type_ref.name),
        ) catch return strip_type_namespace(type_ref.name);
        buf.append(self.arena, '<') catch return strip_type_namespace(type_ref.name);
        for (type_ref.params, 0..) |param, i| {
            if (i > 0) buf.appendSlice(self.arena, ",") catch {};
            buf.appendSlice(self.arena, self.render_type_ref(param)) catch {};
        }
        buf.append(self.arena, '>') catch {};
        return buf.toOwnedSlice(self.arena) catch strip_type_namespace(type_ref.name);
    }

    fn coerce_soql_assignment_to_declared_type(
        self: *Evaluator,
        val: Value,
    ...
    fn eval_assignment(
        ...
        var coerced_val = val;
        if (self.resolve_assignment_target_type(asgn.target, current_env)) |target_type| {
            coerced_val = try self.coerce_soql_assignment_to_declared_type(
                coerced_val,
                asgn.value,
                target_type,
            );
    ...
    /// `record.addError(FieldName, msg)` — attach an error to the field on the
    /// record rather than dereferencing its value. Detecting the pattern
    /// before evaluating the receiver avoids a spurious NPE on null fields.
    fn try_mc_add_error_sugar(
        self: *Evaluator,
        mc: *ast.MethodCallExpr,
        current_env: *Env,
    ) anyerror!?Value {
        if (!std.ascii.eqlIgnoreCase(mc.method, "addError")) return null;
    ...
    /// Null-safe operator (?.): when the receiver evaluates to null (or throws
    /// a NullPointerException), short-circuit the call to null. Returns null
    /// when the call is not null-safe or the receiver is non-null.
    fn mc_null_safe_short_circuit(
        self: *Evaluator,
        mc: *ast.MethodCallExpr,
        current_env: *Env,
    ) anyerror!?Value {
        if (!mc.null_safe) return null;
    ...
    /// `System.Search.query(soql)` short-circuit: execute SOSL directly.
    /// Returns null when the method call is not this specific chain.
    fn mc_try_system_search_query(
        self: *Evaluator,
        mc: *ast.MethodCallExpr,
        args: []const Value,
    ) anyerror!?Value {
        ...
        if (search_fa.object.* != .identifier) return null;
        if (!(std.ascii.eqlIgnoreCase(search_fa.object.identifier.name, "System") and
    ...
    /// `super.method(...)` dispatch: resolve the super class of the current
    /// class and invoke the method against `this`.
    fn mc_dispatch_super_call(
        self: *Evaluator,
        mc: *ast.MethodCallExpr,
        args: []const Value,
        current_env: *Env,
    ) anyerror!Value {
        const this_val = current_env.get("this") orelse return Value.null_val;
        if (this_val != .object) return Value.null_val;
        const current_class_name = self.current_class orelse return Value.null_val;
        const current_decl = self.find_class(current_class_name) orelse return Value.null_val;
        const sc = current_decl.super_class orelse return Value.null_val;
        const super_decl = self.find_class(sc.name) orelse return Value.null_val;
    ...
    /// Dispatch `SomeIdentifier.method(args)` where the receiver is a bare
    /// identifier. Tries in order: named builtins (Assert/Test/JSON/__mdt/
    /// Integer.valueOf), local variable bound to `class_name`, shadowing user
    /// class, Database, builtin static dispatch, Custom Metadata getInstance/
    /// getAll, Custom Setting statics, user class call, Test factory stubs,
    /// getSObjectType fallback, and finally implicit `this` instance method.
    fn mc_dispatch_identifier(
        self: *Evaluator,
        mc: *ast.MethodCallExpr,
        args: []const Value,
        current_env: *Env,
        ...
        if (try self.mc_try_named_builtin_dispatch(class_name, mc.method, args, current_env)) |v| {
            return v;
        }
        if (try self.mc_try_resolved_variable_dispatch(mc, args, current_env)) |v| return v;
        if (try self.mc_try_user_class_shadow_dispatch(mc, class_name, args, current_env)) |v| {
            return v;
        }
        if (std.ascii.eqlIgnoreCase(class_name, "Database")) {
            return self.handle_database_method(mc.method, args, current_env);
        }
        var bctx = builtins.BuiltinContext{
            .arena = self.arena,
            .stdout = &self.stdout,
        ...
        if (try builtins.dispatch_static(&bctx, class_name, mc.method, args)) |result| {
            return result;
        }
        if (try self.mc_try_custom_metadata_dispatch(class_name, mc.method, args)) |v| return v;
        if (try self.handle_custom_setting_static_method(class_name, mc.method, args)) |r| {
            return r;
        }
        if (!std.ascii.eqlIgnoreCase(class_name, "Database") and
            self.find_class(class_name) != null)
        {
            return self.call_method(class_name, mc.method, args);
        }
        if (try self.handle_test_factory(class_name, mc.method, args)) |result| return result;
        if (std.ascii.eqlIgnoreCase(mc.method, "getSObjectType")) {
            return try self.make_sobject_type_token(class_name);
        }
        // Unknown static target — the identifier may resolve to an instance
        // value via implicit `this`. Route through eval_expr so instance
        // property getters fire.
        if (self.find_class(class_name) == null) {
            if (self.eval_expr(mc.object, current_env)) |obj_val| {
                if (obj_val != .null_val) {
                    return self.eval_instance_method(obj_val, mc.method, args, current_env);
                }
            } else |_| {}
        }
        return self.call_method(class_name, mc.method, args);
    ...
    /// Dispatch when the identifier resolves to a local variable or a static
    /// field with a primitive/reference value. Returns null when no binding
    /// matches (the caller falls through to the static class dispatch).
    fn mc_try_resolved_variable_dispatch(
        self: *Evaluator,
        mc: *ast.MethodCallExpr,
        args: []const Value,
        current_env: *Env,
        ...
        const class_name = mc.object.identifier.name;
        const resolved_var = try self.mc_resolve_identifier_variable(class_name, current_env);
        switch (resolved_var) {
        ...
        if (resolved_var == .null_val and current_env.has(class_name)) {
            ...
            return try self.eval_instance_method(rebound, mc.method, args, current_env);
        }
        return null;
    }

    /// When a platform static namespace (e.g. `Messaging`, `Http`) is shadowed
    /// by a user-defined class visible in scope, dispatch to the user class
    /// instead. Returns null when no shadowing applies.
    fn mc_try_user_class_shadow_dispatch(
        self: *Evaluator,
        mc: *ast.MethodCallExpr,
        class_name: []const u8,
        args: []const Value,
        ...
        if (!is_builtin_static_namespace(class_name)) return null;
        const visible_class = self.resolve_visible_user_class_in_scope(
            current_env,
            class_name,
        ) orelse
            return null;
        return try self.call_method(visible_class, mc.method, args);
    }

    /// Dispatch for named builtin classes: Assert, Test, JSON, __mdt,
    /// Integer.valueOf, System.enqueue_job/schedule/assert*. Returns null
    /// when the identifier does not match any known builtin.
    fn mc_try_named_builtin_dispatch(
        self: *Evaluator,
        class_name: []const u8,
        method: []const u8,
        args: []const Value,
        ...
        if (std.ascii.eqlIgnoreCase(class_name, "Test")) {
            return try self.handle_test(method, args);
        }
        if (std.ascii.eqlIgnoreCase(class_name, "System") and
            std.ascii.eqlIgnoreCase(method, "enqueue_job"))
        {
            if (args.len > 0 and args[0] == .object) return try self.enqueue_job(args[0].object);
            return .void_val;
        }
        if (std.ascii.eqlIgnoreCase(class_name, "System") and
            std.ascii.eqlIgnoreCase(method, "schedule"))
        {
            return try self.handle_system_method("schedule", method, args, current_env);
        }
        if (std.ascii.eqlIgnoreCase(class_name, "JSON")) {
            if (try self.mc_try_json_builtin(method, args)) |v| return v;
        }
        if (try self.mc_try_custom_metadata_dispatch(class_name, method, args)) |v| return v;
        if (try self.mc_try_integer_valueof(class_name, method, args)) |v| return v;
        return null;
    }

    fn mc_try_integer_valueof(
        self: *Evaluator,
        class_name: []const u8,
        method: []const u8,
        args: []const Value,
    ) anyerror!?Value {
        if (!std.ascii.eqlIgnoreCase(class_name, "Integer")) return null;
        if (!std.ascii.eqlIgnoreCase(method, "valueOf")) return null;
        if (args.len == 0 or args[0] != .string) return null;
        if (std.fmt.parseInt(i64, args[0].string, 10)) |v| {
    ...
    /// JSON.serialize / deserialize / deserializeStrict / createParser.
    fn mc_try_json_builtin(
        self: *Evaluator,
        method: []const u8,
        args: []const Value,
    ) anyerror!?Value {
        ...
        {
            return try self.mc_json_deserialize(args);
        }
    ...
    fn mc_json_serialize(
        self: *Evaluator,
        args: []const Value,
    ) anyerror!Value {
        if (args.len == 0) return Value{ .string = "{}" };
        if (args[0] == .object and (std.ascii.eqlIgnoreCase(
            args[0].object.class_name,
            "Schema.SObjectField",
        ) or std.ascii.eqlIgnoreCase(args[0].object.class_name, "SObjectField"))) {
            const exc = try self.arena.create(types.ObjectInstance);
            exc.* = .{ .class_name = "System.JSONException" };
            try exc.fields.put(self.arena, "message", Value{
                .string = "Apex Type unsupported in JSON: Schema.SObjectField",
            });
            self.pending_exception = Value{ .object = exc };
            return error.ApexException;
        }
    ...
    fn mc_json_deserialize(
        self: *Evaluator,
        args: []const Value,
    ) anyerror!Value {
        if (args.len >= 1 and args[0] == .string) {
            return try self.mc_json_deserialize_string(args);
        }
    ...
    fn mc_json_deserialize_string(
        ...
        {
            return try self.raise_json_exception("JSONException", json_str);
        }
        const type_name: []const u8 = if (args.len >= 2 and args[1] == .object) blk: {
            const tobj = args[1].object;
        ...
        if (!utils.is_json_balanced(trimmed_json)) {
            return try self.raise_json_exception("System.JSONException", json_str);
        }
    ...
    /// Custom Metadata Type: `MyType__mdt.getInstance(name)` / `getAll()`.
    /// Lazily loads the store from `.md-meta.xml` on first access, then
    /// resolves developer names case-insensitively. Returns null when the
    /// identifier is not a `__mdt` type or the method is not supported.
    fn mc_try_custom_metadata_dispatch(
        self: *Evaluator,
        class_name: []const u8,
        method: []const u8,
        args: []const Value,
        ...
        if (!(std.ascii.eqlIgnoreCase(method, "getInstance") or
        ...
        if (std.ascii.eqlIgnoreCase(method, "getInstance")) {
            ...
            return self.mc_find_custom_metadata_record(class_name, dev_name);
        }
        // getAll()
        const map = try self.arena.create(types.MapValue);
        map.* = .{};
    ...
    fn ensure_custom_metadata_loaded(self: *Evaluator, class_name: []const u8) void {
        ...
        if (!found) self.load_custom_metadata_from_files(class_name) catch {};
    }

    fn mc_find_custom_metadata_record(
        self: *Evaluator,
        class_name: []const u8,
        dev_name: []const u8,
    ) Value {
    ...
    /// Resolve a bare identifier to a local, static property getter output,
    /// or static field value inherited from the class chain / outer classes.
    /// Returns null_val when nothing matches.
    fn mc_resolve_identifier_variable(
        self: *Evaluator,
        class_name: []const u8,
        current_env: *Env,
    ) anyerror!Value {
        if (current_env.get(class_name)) |v| return v;
        if (self.current_class) |cc| {
            if (try self.mc_resolve_static_getter_on_class(cc, class_name)) |v| return v;
            const key = std.fmt.allocPrint(
                self.arena,
                "{s}.{s}",
                .{ cc, class_name },
            ) catch return Value.null_val;
            if (self.global_env.get(key)) |v| return v;
            if (self.resolve_outer_static_field(cc, class_name)) |v| return v;
        }
        if (current_env.get("this")) |this_val| {
            if (this_val == .object) {
                if (try self.mc_resolve_static_from_this(
                    this_val.object.class_name,
                    class_name,
                )) |v| {
                    return v;
    ...
    fn mc_resolve_static_getter_on_class(
        self: *Evaluator,
        cc: []const u8,
        class_name: []const u8,
    ) anyerror!?Value {
        const already_in_getter = if (self.evaluating_getter) |eg|
            std.ascii.eqlIgnoreCase(eg, class_name)
        else
            false;
        if (already_in_getter) return null;
        const cd = self.find_class(cc) orelse return null;
        for (cd.members) |member| {
            switch (member) {
                .field_decl => |fd| {
                    if (!(fd.modifiers.is_static and
                        std.ascii.eqlIgnoreCase(fd.name, class_name) and
                        fd.getter_body != null)) continue;
                    ...
                    const result = self.exec_block(fd.getter_body.?, getter_env) catch
                        return Value.null_val;
    ...
    fn mc_resolve_static_from_this(
        self: *Evaluator,
        this_cn: []const u8,
        class_name: []const u8,
    ) anyerror!?Value {
        ...
        if (self.find_class(this_cn)) |this_cd| {
            ...
            while (cur_parent) |sc| {
                ...
                if (self.global_env.get(pkey)) |v| return v;
                cur_parent = if (self.find_class(sc.name)) |pcd| pcd.super_class else null;
            }
        }
        if (self.resolve_outer_static_field(this_cn, class_name)) |v| return v;
        return null;
    }
    ...
    fn eval_method_call(
        ...
        if (try self.try_mc_add_error_sugar(mc, current_env)) |v| return v;
        if (try self.mc_null_safe_short_circuit(mc, current_env)) |v| return v;
        ...
        if (try self.mc_try_system_search_query(mc, args.items)) |v| return v;
        if (mc.object.* == .super_expr) {
            return try self.mc_dispatch_super_call(mc, args.items, current_env);
        }
        if (mc.object.* == .identifier) {
            return try self.mc_dispatch_identifier(mc, args.items, current_env);
        }
        if (mc.object.* == .field_access) {
            if (try self.mc_dispatch_nested_field_access(mc, args.items, current_env)) |v| {
                return v;
            }
        }
    ...
    /// Dispatch for chains of the form `Outer.Inner.method(...)` (three-level
    /// field_access on the receiver). Covers Schema.<X>.SObjectType.<m>,
    /// System.Assert.*, System.<ns>.<m>, <X>.<Y>.getDescribe, DataWeave.Script,
    /// Invocable.Action, Flow.Interview, ConnectApi.*, and Cache.Session/Org.
    /// Returns null when no branch applies.
    fn mc_dispatch_nested_field_access(
        self: *Evaluator,
        mc: *ast.MethodCallExpr,
        args: []const Value,
        current_env: *Env,
        ...
        const fa = mc.object.field_access;
        if (fa.object.* == .field_access) {
            if (try self.mc_try_three_level_schema(mc, fa, args, current_env)) |v| return v;
        }
        if (fa.object.* != .identifier) return null;
        const outer_class = fa.object.identifier.name;
        const inner = fa.field;

        if (std.ascii.eqlIgnoreCase(outer_class, "System") and
            std.ascii.eqlIgnoreCase(inner, "Assert"))
        {
            return try self.handle_assert(mc.method, args);
        }
        if (std.ascii.eqlIgnoreCase(outer_class, "System")) {
            return try self.handle_system_method(inner, mc.method, args, current_env);
        }
        if (std.ascii.eqlIgnoreCase(mc.method, "getDescribe")) {
            return try self.mc_get_describe_for_token(mc, outer_class, inner, args, current_env);
        }
        if (try self.mc_try_dataweave_script(outer_class, inner, mc.method, args)) |v| return v;
        if (try self.mc_try_invocable_action(outer_class, inner, mc.method, args)) |v| return v;
        if (try self.mc_try_flow_interview(outer_class, inner, mc.method, args)) |v| return v;
        if (std.ascii.eqlIgnoreCase(outer_class, "ConnectApi")) {
            return (try self.handle_connect_api_namespace(inner, mc.method, args)) orelse
                Value.null_val;
        }
        if (try self.mc_try_cache_partition(outer_class, inner, mc.method, args)) |v| return v;
        return null;
    }

    /// Three-level `Schema.TypeName.SObjectType.X` / `Schema.TypeName.Field.
    /// getDescribe()` chains. Returns null when the outer isn't `Schema`.
    fn mc_try_three_level_schema(
        self: *Evaluator,
        mc: *ast.MethodCallExpr,
        fa: *ast.FieldAccess,
        args: []const Value,
        ...
        if (std.ascii.eqlIgnoreCase(type_name, "SObjectType")) {
            const sot = try self.make_sobject_type_token(fa.field);
            return try self.eval_instance_method(sot, mc.method, args, current_env);
        }
        if (std.ascii.eqlIgnoreCase(fa.field, "SObjectType")) {
            const sot = try self.make_sobject_type_token(type_name);
            return try self.eval_instance_method(sot, mc.method, args, current_env);
        }
        if (std.ascii.eqlIgnoreCase(mc.method, "getDescribe")) {
            var bctx = builtins.BuiltinContext{
            ...
            return try builtins.create_field_describe_result(&bctx, type_name, fa.field);
        }
        return null;
    }

    fn mc_get_describe_for_token(
        self: *Evaluator,
        mc: *ast.MethodCallExpr,
        ...
        if (std.ascii.eqlIgnoreCase(inner, "SObjectType")) {
            const sot = try self.make_sobject_type_token(outer_class);
            return try self.eval_instance_method(sot, mc.method, args, current_env);
        }
        var bctx = builtins.BuiltinContext{
            .arena = self.arena,
        ...
        return try builtins.create_field_describe_result(&bctx, outer_class, inner);
    }

    fn mc_try_dataweave_script(
        self: *Evaluator,
        outer_class: []const u8,
        inner: []const u8,
        method: []const u8,
        ...
        if (!(std.ascii.eqlIgnoreCase(outer_class, "DataWeave") and
            std.ascii.eqlIgnoreCase(inner, "Script"))) return null;
        if (!(std.ascii.eqlIgnoreCase(method, "createScript") and
            args.len > 0 and args[0] == .string)) return null;
        const dw = try self.arena.create(types.ObjectInstance);
        dw.* = .{ .class_name = "DataWeave.Script" };
        try dw.fields.put(self.arena, "scriptName", args[0]);
        return Value{ .object = dw };
    }

    fn mc_try_invocable_action(
        self: *Evaluator,
        outer_class: []const u8,
        inner: []const u8,
        method: []const u8,
        ...
        if (!(std.ascii.eqlIgnoreCase(outer_class, "Invocable") and
            std.ascii.eqlIgnoreCase(inner, "Action"))) return null;
        var bctx = builtins.BuiltinContext{
            .arena = self.arena,
            .stdout = &self.stdout,
            .pending_exception = &self.pending_exception,
        ...
        return try builtins.dispatch_static(&bctx, "Invocable.Action", method, args);
    }

    fn mc_try_flow_interview(
        self: *Evaluator,
        outer_class: []const u8,
        inner: []const u8,
        method: []const u8,
        ...
        if (!(std.ascii.eqlIgnoreCase(outer_class, "Flow") and
            std.ascii.eqlIgnoreCase(inner, "Interview"))) return null;
        if (!std.ascii.eqlIgnoreCase(method, "createInterview")) return null;
        if (args.len == 0 or args[0] != .string) return null;
        ...
        if (args.len > 1 and args[1] == .map) {
            for (args[1].map.entries.keys(), args[1].map.entries.values()) |k, v| {
    ...
    fn mc_try_cache_partition(
        ...
        if (!std.ascii.eqlIgnoreCase(outer_class, "Cache")) return null;
        if (!(std.ascii.eqlIgnoreCase(inner, "Session") or
            std.ascii.eqlIgnoreCase(inner, "Org"))) return null;
        if (!std.ascii.eqlIgnoreCase(method, "getPartition")) return null;
    ...
    fn prepare_method_arg_value(self: *Evaluator, value: Value) !Value {
        return switch (value) {
    ...
    fn connect_api_markup_tag(markup_value: Value) ?[]const u8 {
    ...
    fn is_connect_api_hashtag_char(ch: u8) bool {
        return std.ascii.isAlphanumeric(ch) or ch == '_';
    }

    fn is_connect_api_whitespace(ch: u8) bool {
        return ch == ' ' or ch == '\t' or ch == '\n' or ch == '\r';
    }

    fn is_connect_api_mention_id(self: *Evaluator, record_id: []const u8) bool {
        _ = self;
    ...
    fn connect_api_class_name_equals(class_name: []const u8, simple_name: []const u8) bool {
    ...
    fn connect_api_append_output_segment(
        self: *Evaluator,
        segments: *types.ListValue,
        segment: *types.ObjectInstance,
    ) !void {
        try segments.items.append(self.arena, Value{ .object = segment });
    }

    fn connect_api_append_text_segment(
        self: *Evaluator,
        segments: *types.ListValue,
        text: []const u8,
    ) !void {
        if (text.len == 0) return;
        const segment = try self.connect_api_create_object("ConnectApi.TextSegment");
        try segment.fields.put(self.arena, "text", Value{ .string = text });
        try self.connect_api_append_output_segment(segments, segment);
    }

    fn connect_api_append_parsed_text(
        self: *Evaluator,
        segments: *types.ListValue,
        text: []const u8,
    ) !void {
        ...
        var i: usize = 0;
        while (i < text.len) {
            const is_http = std.mem.startsWith(
                u8,
                text[i..],
                "http://",
            ) or std.mem.startsWith(u8, text[i..], "https://");
            if (is_http) {
                try self.connect_api_append_text_segment(segments, text[text_start..i]);
                var end = i;
                while (end < text.len and !is_connect_api_whitespace(text[end])) : (end += 1) {}
                ...
                try self.connect_api_append_output_segment(segments, link);
                i = end;
                text_start = end;
                continue;
            }

            if (text[i] == '#' and i + 1 < text.len and is_connect_api_hashtag_char(text[i + 1])) {
                try self.connect_api_append_text_segment(segments, text[text_start..i]);
                var end = i + 1;
                while (end < text.len and is_connect_api_hashtag_char(text[end])) : (end += 1) {}
                ...
                try self.connect_api_append_output_segment(segments, hashtag);
                i = end;
                text_start = end;
                continue;
            }
        ...
        try self.connect_api_append_text_segment(segments, text[text_start..]);
    }

    fn connect_api_build_output_segments(
        self: *Evaluator,
        message_segments: Value,
    ) !*types.ListValue {
    ...
    fn connect_api_render_body_text(
        self: *Evaluator,
        output_segments: *types.ListValue,
    ) ![]const u8 {
        var buf: std.ArrayListUnmanaged(u8) = .empty;
    ...
    fn alloc_unique_collection_key(self: *Evaluator) ![]const u8 {
    ...
    fn find_map_entry_key(self: *Evaluator, map: *types.MapValue, key_value: Value) ?[]const u8 {
        ...
        for (map.key_values.keys(), map.key_values.values()) |stored_key, original_key| {
            if (self.values_equal(original_key, key_value)) return stored_key;
        }
    ...
    fn set_entry_key(self: *Evaluator, value: Value) ![]const u8 {
        return switch (value) {
            .sobject => |sob| blk: {
                ...
                if (utils.sobject_get(&sob.fields, "UniqueId__c")) |unique_id| {
                    if (unique_id == .string) {
                        break :blk try std.fmt.allocPrint(
                            self.arena,
                            "{s}#uid:{s}",
                            .{ sob.type_name, unique_id.string },
                        );
                    }
                }
                if (utils.sobject_get(&sob.fields, "Name")) |name_val| {
                    if (name_val == .string) {
                        break :blk try std.fmt.allocPrint(
                            self.arena,
                            "{s}#name:{s}",
                            .{ sob.type_name, name_val.string },
                        );
                    }
                }
            ...
            .object, .list, .map, .set => try self.alloc_unique_collection_key(),
            else => try utils.coerce_to_string(value, self.arena),
        };
    }

    fn find_set_entry_key(self: *Evaluator, set: *types.SetValue, value: Value) ?[]const u8 {
        if (!self.uses_unique_collection_key(value)) {
            const rendered = self.set_entry_key(value) catch null;
            if (rendered) |key| {
        ...
        for (set.entries.keys(), set.entries.values()) |stored_key, existing_value| {
            if (self.values_equal(existing_value, value)) return stored_key;
        }
    ...
    fn eval_set_membership_method(
        ...
        if (std.ascii.eqlIgnoreCase(method, "equals") and args.len > 0) {
            return Value{ .boolean = self.values_equal(Value{ .set = set }, args[0]) };
        }
        if (std.ascii.eqlIgnoreCase(method, "hashCode")) {
            return Value{ .integer = try self.value_hash_code(Value{ .set = set }) };
        }
        if (std.ascii.eqlIgnoreCase(method, "contains") and args.len > 0) {
    ...
    fn eval_set_mutation_method(
        ...
        if (std.ascii.eqlIgnoreCase(method, "remove") and args.len > 0) {
            if (self.find_set_entry_key(set, args[0])) |key| {
    ...
    fn eval_string_method_substring(s: []const u8, args: []const Value) !?Value {
    ...
    fn append_split_literal(
        ...
        if (split_limit) |limit| {
            if (limit <= 1) {
                try list.items.append(arena, Value{ .string = s });
                return;
            }
            var start: usize = 0;
            ...
            while (splits_done + 1 < limit) {
                ...
                try list.items.append(arena, Value{ .string = s[start..next] });
                start = next + pattern.len;
                splits_done += 1;
            }
            try list.items.append(arena, Value{ .string = s[start..] });
            return;
        }
        var iter = std.mem.splitSequence(u8, s, pattern);
        while (iter.next()) |part| {
            try list.items.append(arena, Value{ .string = part });
        }
    }

    fn eval_string_method_split(self: *Evaluator, s: []const u8, args: []const Value) !Value {
        const list = try self.arena.create(types.ListValue);
        list.* = .{};
        ...
        // Apex's String.split(regex) treats the argument as a regex.
        const regex_chars = "\\[].|?*+()^${}";
        ...
        if (has_regex_meta) {
            if (split_by_regex(self.arena, pattern, s, split_limit)) |parts| {
                ...
                // Regex engine rejected — fall through to literal split.
            }
        }
        try append_split_literal(self.arena, list, s, pattern, split_limit);
        return Value{ .list = list };
    }

    fn eval_string_method_replace_all_tail(
        trimmed_result: []const u8,
        trimmed_input: []const u8,
    ) ?Value {
        if (std.mem.eql(u8, trimmed_result, trimmed_input)) return null;
        if (std.mem.indexOf(u8, trimmed_input, "Class.") == null) return null;
        const anonymous_tail = "AnonymousBlock: line 1, column 1";
        if (std.mem.eql(u8, trimmed_result, anonymous_tail)) return Value{ .string = "" };
        if (!std.mem.startsWith(u8, trimmed_result, "Class.")) return null;
        const orphaned_tail = std.mem.trim(u8, trimmed_result["Class.".len..], " \t\r\n");
        if (std.mem.eql(u8, orphaned_tail, anonymous_tail)) return Value{ .string = "" };
        return null;
    }

    fn eval_string_method_replace_all(self: *Evaluator, s: []const u8, args: []const Value) !Value {
        ...
        if (eval_string_method_replace_all_tail(trimmed_result, trimmed_input)) |v| return v;
        return Value{ .string = result };
    }

    fn parse_numeric_arg(arg: Value, arena: std.mem.Allocator) ?usize {
        return switch (arg) {
            ...
            .string => |arg_s| blk: {
                ...
                const arg_s = utils.coerce_to_string(arg, arena) catch break :blk null;
                if (std.fmt.parseInt(i64, arg_s, 10)) |parsed| {
                    break :blk @intCast(@max(parsed, 0));
                } else |_| {}
                if (std.fmt.parseFloat(f64, arg_s)) |parsed| {
    ...
    fn eval_string_method_pad_abbrev(
        self: *Evaluator,
        s: []const u8,
        method: []const u8,
        args: []const Value,
        ...
        if (std.ascii.eqlIgnoreCase(method, "left") and args.len > 0) {
            const n: usize = parse_numeric_arg(
                args[0],
                self.arena,
            ) orelse return Value{ .string = s };
        ...
        if (std.ascii.eqlIgnoreCase(method, "abbreviate") and
            args.len > 0 and args[0] == .integer)
        {
            ...
            const prefix = s[0 .. max_width - 3];
            return Value{ .string = try std.fmt.allocPrint(self.arena, "{s}...", .{prefix}) };
        }
        if (std.ascii.eqlIgnoreCase(method, "leftPad") and args.len > 0 and args[0] == .integer) {
            ...
            const pad_char: u8 = if (args.len > 1 and args[1] == .string and
                args[1].string.len > 0) args[1].string[0] else ' ';
    ...
    fn eval_string_method_predicates(s: []const u8, method: []const u8) ?Value {
    ...
    fn all_chars_match(s: []const u8, pred: fn (u8) bool) bool {
    ...
    fn eval_string_method_char_class(self: *Evaluator, s: []const u8, method: []const u8) !?Value {
        if (std.ascii.eqlIgnoreCase(method, "isNumeric")) {
            return Value{ .boolean = all_chars_match(s, std.ascii.isDigit) };
        }
        if (std.ascii.eqlIgnoreCase(method, "isAlpha")) {
            return Value{ .boolean = all_chars_match(s, std.ascii.isAlphabetic) };
        }
        if (std.ascii.eqlIgnoreCase(method, "isAlphanumeric")) {
            return Value{ .boolean = all_chars_match(s, std.ascii.isAlphanumeric) };
        }
    ...
    fn eval_string_method_remove(s: []const u8, method: []const u8, args: []const Value) ?Value {
        if (args.len == 0 or args[0] != .string) return null;
        if (std.ascii.eqlIgnoreCase(method, "removeEnd")) {
            if (std.mem.endsWith(u8, s, args[0].string)) {
                return Value{ .string = s[0 .. s.len - args[0].string.len] };
            }
            return Value{ .string = s };
        }
        if (std.ascii.eqlIgnoreCase(method, "removeStart")) {
    ...
    fn eval_string_method_format(self: *Evaluator, s: []const u8, args: []const Value) !Value {
                ...
                if (s[i] == '{' and i + 1 < s.len) {
                    if (std.mem.indexOfScalarPos(u8, s, i + 1, '}')) |close| {
                        ...
                        if (std.fmt.parseInt(usize, idx_str, 10)) |idx| {
                            if (idx < items.len) {
                                const val_str: []const u8 = try utils.coerce_to_string(
                                    items[idx],
                                    self.arena,
                                );
    ...
    fn eval_string_method_date_accessor(
        self: *Evaluator,
        s: []const u8,
        method: []const u8,
    ) !?Value {
        ...
        if (std.ascii.eqlIgnoreCase(method, "getTime")) {
            ...
            const is_leap: i64 = if (@mod(y, 4) == 0 and
                (@mod(y, 100) != 0 or @mod(y, 400) == 0)) @as(i64, 1) else 0;
    ...
    fn coerce_i32_arg(args: []const Value) i32 {
    ...
    fn eval_string_method_add_hours(
        self: *Evaluator,
        s: []const u8,
        args: []const Value,
    ) anyerror!Value {
        ...
        return Value{ .string = try format_iso_datetime(
            self.arena,
            y,
            dt.m,
            dt.d,
    ...
    fn eval_string_method_add_minutes(
        self: *Evaluator,
        s: []const u8,
        args: []const Value,
    ) anyerror!Value {
        ...
        if (hour_offset != 0) {
            return try self.eval_string_method(
                base,
                "addHours",
    ...
    fn eval_string_method_add_seconds(
        self: *Evaluator,
        s: []const u8,
        args: []const Value,
    ) anyerror!Value {
        ...
        if (min_offset != 0) {
            return try self.eval_string_method(
                base,
                "addMinutes",
                &.{Value{ .integer = min_offset }},
            );
        }
        return Value{ .string = base };
    }

    fn eval_string_method_add_hms(
        self: *Evaluator,
        s: []const u8,
        method: []const u8,
        args: []const Value,
        ...
        if (std.ascii.eqlIgnoreCase(
            method,
            "addHours",
        )) return try self.eval_string_method_add_hours(s, args);
        if (std.ascii.eqlIgnoreCase(
            method,
            "addMinutes",
        )) return try self.eval_string_method_add_minutes(s, args);
        if (std.ascii.eqlIgnoreCase(
            method,
            "addSeconds",
        )) return try self.eval_string_method_add_seconds(s, args);
        return null;
    }

    fn eval_string_method_date_component(s: []const u8, method: []const u8) ?Value {
        if (std.ascii.eqlIgnoreCase(method, "year") or
            std.ascii.eqlIgnoreCase(method, "yearGmt"))
        {
            const dt = parse_iso_date(s) orelse return Value.null_val;
            return Value{ .integer = dt.y };
        }
        if (std.ascii.eqlIgnoreCase(method, "month") or
            std.ascii.eqlIgnoreCase(method, "monthGmt"))
        {
            const dt = parse_iso_date(s) orelse return Value.null_val;
            return Value{ .integer = dt.m };
        }
        if (std.ascii.eqlIgnoreCase(method, "day") or
        ...
        if (std.ascii.eqlIgnoreCase(method, "millisecond") or
            std.ascii.eqlIgnoreCase(method, "millisecondGmt"))
        {
            _ = parse_iso_date(s) orelse return Value{ .integer = 0 };
            return Value{ .integer = 0 };
        }
        if (std.ascii.eqlIgnoreCase(method, "day_of_year")) {
            const dt = parse_iso_date(s) orelse return Value.null_val;
            const is_leap =
    ...
    fn arg_to_date_string(s: []const u8, arg: Value) []const u8 {
        return switch (arg) {
            .string => |os| os,
            .object => |obj| blk: {
    ...
    fn eval_string_method_date_diff(s: []const u8, method: []const u8, args: []const Value) ?Value {
        if (args.len == 0) return null;
        if (std.ascii.eqlIgnoreCase(method, "daysBetween")) {
            const other_str = arg_to_date_string(s, args[0]);
        ...
        if (std.ascii.eqlIgnoreCase(method, "monthsBetween")) {
            const other_str = arg_to_date_string(s, args[0]);
            const a = parse_iso_date(s) orelse return Value.null_val;
            const b = parse_iso_date(other_str) orelse return Value.null_val;
    ...
    fn eval_string_method_format_gmt(self: *Evaluator, s: []const u8, args: []const Value) !Value {
        if (args.len > 0 and args[0] == .string) {
            return self.format_date_time_pattern(s, args[0].string);
        }
    ...
    fn eval_string_method_enum_value_of(
        self: *Evaluator,
        s: []const u8,
        args: []const Value,
    ) !Value {
    ...
    fn resolve_sobject_type_from_id(self: *Evaluator, s: []const u8) []const u8 {
    ...
    fn eval_string_method_get_sobject_type(self: *Evaluator, s: []const u8) !Value {
        const type_name = self.resolve_sobject_type_from_id(s);
    ...
    fn eval_string_method_enum_values(self: *Evaluator, s: []const u8) !?Value {
        var class_iter = self.classes.iterator();
        while (class_iter.next()) |entry| {
            for (entry.value_ptr.*.members) |member| {
                switch (member) {
                    .enum_decl => |ed| {
                        if (!std.ascii.eqlIgnoreCase(ed.name, s)) continue;
                        const list = try self.arena.create(types.ListValue);
                        list.* = .{};
                        for (ed.values) |v| {
                            try list.items.append(self.arena, Value{ .string = v });
    ...
    fn eval_string_method_misc(
        ...
        if (std.ascii.eqlIgnoreCase(method, "name") or
            std.ascii.eqlIgnoreCase(method, "toString"))
        {
            return Value{ .string = s };
        }
        ...
        if (std.ascii.eqlIgnoreCase(method, "isSameDay") and args.len > 0) {
            const other_str = builtins.extract_date_string(args[0]) orelse
                (if (args[0] == .string) args[0].string else "");
        ...
        if (std.ascii.eqlIgnoreCase(method, "dateGmt") or
            std.ascii.eqlIgnoreCase(method, "dateGMT"))
        {
            const dt = parse_iso_date(s) orelse return Value{ .string = s };
            return Value{ .string = try std.fmt.allocPrint(
    ...
    fn eval_string_method_char_ops(
        self: *Evaluator,
        s: []const u8,
        method: []const u8,
        args: []const Value,
        ...
        if (std.ascii.eqlIgnoreCase(method, "starts_with_ignore_case") and
            args.len > 0 and args[0] == .string)
        {
    ...
    fn eval_string_method_substring_family(
        s: []const u8,
        method: []const u8,
        args: []const Value,
    ) ?Value {
        if (args.len == 0 or args[0] != .string) return null;
        const sep = args[0].string;
        if (std.ascii.eqlIgnoreCase(method, "substringAfter")) {
            if (std.mem.indexOf(u8, s, sep)) |idx| {
                return Value{ .string = s[idx + sep.len ..] };
            }
            return Value{ .string = "" };
        }
        if (std.ascii.eqlIgnoreCase(method, "substringBefore")) {
            if (std.mem.indexOf(u8, s, sep)) |idx| {
                return Value{ .string = s[0..idx] };
            }
            return Value{ .string = s };
        }
        if (std.ascii.eqlIgnoreCase(method, "substringAfterLast")) {
        ...
        if (std.ascii.eqlIgnoreCase(method, "substringBeforeLast")) {
    ...
    fn eval_string_method_unescape(self: *Evaluator, s: []const u8, method: []const u8) !?Value {
    ...
    fn eval_string_method_replace_equals(
        self: *Evaluator,
        s: []const u8,
        method: []const u8,
        args: []const Value,
    ...
    fn eval_string_method_last_index_of(
        s: []const u8,
        method: []const u8,
        args: []const Value,
    ) ?Value {
    ...
    fn eval_string_method_date_ops(
        self: *Evaluator,
        s: []const u8,
        method: []const u8,
        args: []const Value,
        ...
        if (eval_string_method_date_component(s, method)) |v| return v;
    ...
    fn eval_string_method(
        ...
        if (eval_string_method_substring_family(s, method, args)) |v| return v;
        if (try self.eval_string_method_char_ops(s, method, args)) |v| return v;
        if (std.ascii.eqlIgnoreCase(method, "getSobjectType") or
            std.ascii.eqlIgnoreCase(method, "getSObjectType"))
        {
            return self.eval_string_method_get_sobject_type(s);
        }
        if (std.ascii.eqlIgnoreCase(method, "values")) {
            if (try self.eval_string_method_enum_values(s)) |v| return v;
        }
        if (try self.eval_string_method_misc(s, method, args)) |v| return v;
        return Value.null_val;
    }
    ...
    /// Handle nested chains rooted at `ApexPages.Severity.X`, `Schema.SObjectType.X`,
    /// `Schema.X.SObjectType`, or `Schema.X.Y` (returning an SObjectField token).
    fn fa_try_apexpages_schema_nested(
        self: *Evaluator,
        fa: *ast.FieldAccess,
    ) anyerror!?Value {
        ...
        if (std.ascii.eqlIgnoreCase(inner_fa.object.identifier.name, "ApexPages") and
    ...
    /// Handle identifiers of the form `Schema.X` whose `.Y` access resolves
    /// to an SObjectType/SObjectField token.
    fn fa_try_schema_prefix_identifier(
        self: *Evaluator,
        fa: *ast.FieldAccess,
    ) anyerror!?Value {
        if (fa.object.* != .identifier) return null;
        const ident = fa.object.identifier.name;
        if (!std.mem.startsWith(u8, ident, "Schema.")) return null;
        const schema_name = ident["Schema.".len..];
        if (std.ascii.eqlIgnoreCase(schema_name, "SObjectType")) {
            return try self.make_sobject_type_token(fa.field);
        }
        if (std.ascii.eqlIgnoreCase(fa.field, "SObjectType")) {
    ...
    /// Global `SObjectType.X` shortcut → Schema.SObjectType { name: X }.
    fn fa_try_sobjecttype_shortcut(
        self: *Evaluator,
        fa: *ast.FieldAccess,
    ) anyerror!?Value {
        if (fa.object.* != .identifier) return null;
        if (!std.ascii.eqlIgnoreCase(fa.object.identifier.name, "SObjectType")) return null;
        return try self.make_sobject_type_token(fa.field);
    }

    /// `System.Search` → a Search proxy object.
    fn fa_try_system_search(self: *Evaluator, fa: *ast.FieldAccess) anyerror!?Value {
        if (fa.object.* != .identifier) return null;
        if (!std.ascii.eqlIgnoreCase(fa.object.identifier.name, "System")) return null;
        if (!std.ascii.eqlIgnoreCase(fa.field, "Search")) return null;
        const search = try self.arena.create(types.ObjectInstance);
        search.* = .{ .class_name = "Search" };
        return Value{ .object = search };
    }

    /// `System.Label.X` / `Label.X` → fallback label name string when metadata
    /// is unavailable.
    fn fa_try_label_identifier(self: *Evaluator, fa: *ast.FieldAccess) anyerror!?Value {
        if (fa.object.* == .field_access) {
    ...
    fn make_sobject_type_token(self: *Evaluator, name: []const u8) anyerror!Value {
        const sot = try self.arena.create(types.ObjectInstance);
        sot.* = .{ .class_name = "Schema.SObjectType" };
        try sot.fields.put(self.arena, "name", Value{ .string = name });
        return Value{ .object = sot };
    }

    /// List field access: auto-unwrap to the first SObject element (single-
    /// record SOQL pattern), expose `.size`, and raise QueryException on empty
    /// SOQL results.
    fn fa_list_access(
        self: *Evaluator,
        fa: *ast.FieldAccess,
        ...
        if (list.items.items.len == 0 and fa.object.* == .soql) {
            const exc = try self.arena.create(types.ObjectInstance);
            exc.* = .{ .class_name = "QueryException" };
            try exc.fields.put(
                self.arena,
    ...
    /// SObject field access: case-insensitive lookup, with SObjectException
    /// when the row was produced by stripInaccessible.
    fn fa_sobject_access(
        self: *Evaluator,
        fa: *ast.FieldAccess,
        sobj: *types.SObject,
    ) anyerror!Value {
        if (self.get_s_object_field_value_case_insensitive(sobj, fa.field)) |v| return v;
        if (sobj.is_stripped) {
            const exc = try self.arena.create(types.ObjectInstance);
    ...
    /// ObjectInstance field access. Checks property getters first, then
    /// built-in proxy types (Schema.SObjectType / FieldDescribeMap /
    /// DescribeSObjectResult), and finally a case-insensitive field lookup.
    fn fa_object_instance_access(
        self: *Evaluator,
        fa: *ast.FieldAccess,
        inst: *types.ObjectInstance,
    ) anyerror!Value {
        if (try self.fa_try_property_getter(fa, inst)) |v| return v;
        if (try self.fa_try_schema_proxy_access(fa, inst)) |v| return v;
    ...
    /// Property getter chain (walks up the class hierarchy). Returns null when
    /// no matching field_decl defines a getter.
    fn fa_try_property_getter(
        self: *Evaluator,
        fa: *ast.FieldAccess,
        inst: *types.ObjectInstance,
    ) anyerror!?Value {
        const cd = self.find_class(inst.class_name) orelse return null;
        var cur_cd: ?*ast.ClassDecl = cd;
        while (cur_cd) |ccd| {
            for (ccd.members) |member| {
                switch (member) {
                    .field_decl => |fd| {
                        if (!std.ascii.eqlIgnoreCase(fd.name, fa.field)) continue;
                        const getter = fd.getter_body orelse continue;
                        return try self.invoke_property_getter_inline(
                            fa,
                            inst,
                            cd,
                            ccd,
    ...
    /// Execute a property getter body with `this` bound to the instance. When
    /// we are already inside this property's own getter (self-reference), fall
    /// back to reading the backing field directly.
    fn invoke_property_getter_inline(
        self: *Evaluator,
        fa: *ast.FieldAccess,
        inst: *types.ObjectInstance,
        owner_cd: *ast.ClassDecl,
        declaring_cd: *ast.ClassDecl,
        fd: *ast.FieldDecl,
        getter: []ast.Stmt,
    ) anyerror!Value {
        if (self.evaluating_getter) |eg| {
            if (std.ascii.eqlIgnoreCase(eg, fd.name)) {
                if (fd.modifiers.is_static) {
                    return self.read_static_backing_value(declaring_cd.name, fd.name);
                }
                if (inst.fields.get(fa.field)) |fv| return fv;
                for (inst.fields.keys(), inst.fields.values()) |fk, fv| {
        ...
        const getter_env = self.global_env.child() catch return Value.null_val;
        getter_env.define("this", Value{ .object = inst }) catch {};
        // Load instance fields as local variables, but SKIP fields that have
        // their own getter (so this.field inside THIS getter still chains
        // through to the property getter).
        for (inst.fields.keys(), inst.fields.values()) |fk, fv| {
            const skip = !std.ascii.eqlIgnoreCase(fk, fd.name) and
                fa_field_has_getter(self, owner_cd, fk);
            if (!skip) {
        ...
        const saved_class = self.current_class;
        const saved_getter = self.evaluating_getter;
        self.current_class = declaring_cd.name;
    ...
    /// Walk `cd` and its parents to check whether any field_decl with the
    /// given name declares a getter_body.
    fn fa_field_has_getter(self: *Evaluator, cd: *ast.ClassDecl, name: []const u8) bool {
        var check_cd: ?*ast.ClassDecl = cd;
        while (check_cd) |ccd2| {
            for (ccd2.members) |m2| {
                switch (m2) {
                    .field_decl => |fd2| {
                        if (std.ascii.eqlIgnoreCase(fd2.name, name) and
                            fd2.getter_body != null) return true;
                    },
                    else => {},
                }
            }
    ...
    /// Schema.SObjectTypeNamespace / Schema.SObjectType / FieldDescribeMap /
    /// DescribeSObjectResult proxy field access. Returns null when the object
    /// is not one of the recognised proxy classes.
    fn fa_try_schema_proxy_access(
        self: *Evaluator,
        fa: *ast.FieldAccess,
        inst: *types.ObjectInstance,
    ) anyerror!?Value {
        if (std.ascii.eqlIgnoreCase(inst.class_name, "Schema.SObjectTypeNamespace")) {
            return try self.make_sobject_type_token(fa.field);
        }
        if (std.ascii.eqlIgnoreCase(inst.class_name, "Schema.SObjectType") and
            std.ascii.eqlIgnoreCase(fa.field, "fieldSets"))
        {
            return try self.fa_build_field_set_collection(inst);
        }
        if (std.ascii.eqlIgnoreCase(inst.class_name, "Schema.SObjectType") and
            std.ascii.eqlIgnoreCase(fa.field, "fields"))
        {
            if (inst.fields.get("fields")) |existing| return existing;
            const object_name = fa_object_name_or_sobject(inst);
            ...
            try inst.fields.put(self.arena, "fields", fv);
            return fv;
        }
        if (std.ascii.eqlIgnoreCase(inst.class_name, "FieldDescribeMap")) {
            if (std.ascii.eqlIgnoreCase(fa.field, "map")) {
                if (inst.fields.get("map")) |m| return m;
            }
            if (inst.fields.get("owner")) |owner_val| {
                if (owner_val == .string) {
        ...
        if ((std.ascii.eqlIgnoreCase(inst.class_name, "DescribeSObjectResult") or
            std.ascii.eqlIgnoreCase(inst.class_name, "Schema.DescribeSObjectResult")) and
            std.ascii.eqlIgnoreCase(fa.field, "fieldSets"))
        {
            return try self.fa_build_field_set_collection(inst);
        }
    ...
    fn fa_build_field_set_collection(
        self: *Evaluator,
        inst: *types.ObjectInstance,
    ) anyerror!Value {
        if (inst.fields.get("fieldSets")) |field_sets| return field_sets;
        const object_name = fa_object_name_or_sobject(inst);
        ...
        try inst.fields.put(self.arena, "fieldSets", field_sets);
        return field_sets;
    }

    fn fa_object_name_or_sobject(inst: *types.ObjectInstance) []const u8 {
        if (inst.fields.get("name")) |name_val| {
            if (name_val == .string) return name_val.string;
        }
        return "SObject";
    }

    /// String field access: Schema enum/token handling, `.length`, and
    /// enum-name-to-value lookup. Returns null when no rule matches (caller
    /// continues with other dispatch branches).
    fn fa_string_access(
        self: *Evaluator,
        fa: *ast.FieldAccess,
        s: []const u8,
        ...
        if (fa.object.* == .field_access) {
                ...
                std.ascii.eqlIgnoreCase(inner_fa.object.identifier.name, "Schema"))
            {
                ...
                return try self.make_s_object_field_token(s, fa.field);
            }
        }
        if (fa.object.* == .identifier and
            std.mem.startsWith(u8, fa.object.identifier.name, "Schema."))
    ...
    /// Resolve `staticBase.identifier.X` where `staticBase` is a static field
    /// of the current class (or its outer class). Returns null when no
    /// matching chain resolves.
    fn fa_try_null_with_static_base(
        self: *Evaluator,
        fa: *ast.FieldAccess,
        obj: Value,
        current_env: *Env,
    ) anyerror!?Value {
        if (obj != .null_val) return null;
        if (fa.object.* != .identifier) return null;
        const base_name = fa.object.identifier.name;
        const cc = self.current_class orelse return null;
        if (self.resolve_static_field_value_on_class(cc, base_name)) |base| {
            if (base != .null_val) return try self.eval_field_access_on_resolved_value(
                base,
                fa.field,
                current_env,
            );
    ...
    /// Static field access of the form `ClassName.field`. Handles Schema
    /// prefixes, Date.today(), enum-like singletons (AccessLevel/AccessType),
    /// RestContext, SObjectType / .class, Trigger context, and user-defined
    /// class/enum lookups.
    fn fa_try_static_class_access(
        self: *Evaluator,
        fa: *ast.FieldAccess,
    ) anyerror!?Value {
        if (fa.object.* != .identifier) return null;
        ...
        if (try self.fa_try_schema_qualified_static(class_name, fa.field)) |v| return v;
        if (try self.fa_try_date_today_static(class_name, fa.field)) |v| return v;
        if (fa_try_access_level_enum_static(class_name, fa.field)) |v| return v;
        if (try self.fa_try_rest_context_static(class_name, fa.field)) |v| return v;
        if (try self.fa_try_sobject_type_literal(class_name, fa.field)) |v| return v;
        if (try self.fa_try_class_literal_from_identifier(class_name, fa.field)) |v| return v;
        ...
        if (std.ascii.eqlIgnoreCase(class_name, "Trigger")) {
            return try self.fa_trigger_context_field(fa.field);
        }
        if (try self.fa_try_global_enum_match(class_name, fa.field)) |v| return v;
        if (try self.fa_try_class_member_static(class_name, fa.field)) |v| return v;
    ...
    fn fa_try_schema_qualified_static(
        self: *Evaluator,
        class_name: []const u8,
        field_name: []const u8,
    ) anyerror!?Value {
    ...
    fn fa_try_date_today_static(
        self: *Evaluator,
        class_name: []const u8,
        field_name: []const u8,
    ) anyerror!?Value {
        if (!std.ascii.eqlIgnoreCase(class_name, "Date")) return null;
        if (!std.ascii.eqlIgnoreCase(field_name, "today")) return null;
    ...
    fn fa_try_access_level_enum_static(class_name: []const u8, field_name: []const u8) ?Value {
    ...
    fn fa_try_sobject_type_literal(
        self: *Evaluator,
        class_name: []const u8,
        field_name: []const u8,
    ) anyerror!?Value {
        if (!std.ascii.eqlIgnoreCase(field_name, "SObjectType")) return null;
    ...
    fn fa_try_class_literal_from_identifier(
        self: *Evaluator,
        class_name: []const u8,
        field_name: []const u8,
    ) anyerror!?Value {
        if (!std.ascii.eqlIgnoreCase(field_name, "class")) return null;
        const type_obj = try self.arena.create(types.ObjectInstance);
        type_obj.* = .{ .class_name = "Type" };
    ...
    fn fa_trigger_context_field(self: *Evaluator, field_name: []const u8) anyerror!Value {
        if (self.trigger_context) |tc| {
            ...
            if (std.ascii.eqlIgnoreCase(field_name, "size"))
                return Value{ .integer = tc.size };
            if (std.ascii.eqlIgnoreCase(field_name, "operationType")) {
                return if (tc.operation_type) |ot| Value{ .string = ot } else Value.null_val;
            }
            return Value.null_val;
        }
        // No active trigger: defer to globally-set values / defaults.
    ...
    /// Enum class match scan: if any loaded class declares an enum whose name
    /// matches `class_name`, the access returns the enum value string.
    fn fa_try_global_enum_match(
        self: *Evaluator,
        class_name: []const u8,
        field_name: []const u8,
    ) anyerror!?Value {
    ...
    /// Static members lookup on a specific class: enum/static field/inner
    /// class/inner interface.
    fn fa_try_class_member_static(
        self: *Evaluator,
        class_name: []const u8,
        field_name: []const u8,
    ) anyerror!?Value {
            ...
            switch (member) {
                .enum_decl => |ed| {
                    if (std.ascii.eqlIgnoreCase(ed.name, field_name)) {
                        // Return the enum name as a string; ClassName.EnumName.VALUE
                        // patterns are resolved by a subsequent field_access matching
                        // the enum name through fa_try_global_enum_match.
                        return Value{ .string = field_name };
                    }
                },
                .field_decl => |fd| {
                    if (!std.ascii.eqlIgnoreCase(fd.name, field_name)) continue;
                    if (!fd.modifiers.is_static) continue;
                    return try self.fa_read_static_field(class_name, fd, field_name);
                },
                .class_decl => |inner_cd| {
    ...
    /// Read a static field value: run the getter body if present (and not
    /// already inside it), otherwise fall back to the global static store.
    fn fa_read_static_field(
        self: *Evaluator,
        class_name: []const u8,
        fd: *ast.FieldDecl,
        field_name: []const u8,
    ) anyerror!Value {
        if (fd.getter_body) |getter_body| {
            const already_in = if (self.evaluating_getter) |eg|
            ...
            if (!already_in) {
                ...
                const result = self.exec_block(getter_body, getter_env) catch
                    return Value.null_val;
                return switch (result) {
                    .return_val => |v| v,
                    else => self.return_value,
    ...
    /// Nested static access chain: `Outer.Inner.X` where the object operand
    /// is itself a `field_access`. Handles inner enums/classes/interfaces,
    /// common System.* pseudo-enums, and Schema.* variants.
    fn fa_try_nested_static_access(
        self: *Evaluator,
        fa: *ast.FieldAccess,
    ) anyerror!?Value {
        if (fa.object.* != .field_access) return null;
        ...
        if (try self.fa_nested_in_user_class(fa, outer_name, inner_name)) |v| return v;
        if (try self.fa_nested_in_system_or_schema(fa, outer_name, inner_name)) |v| return v;
        return null;
    }

    /// Handle the `Outer.Inner.X` chain where `Outer` resolves to a user class.
    fn fa_nested_in_user_class(
        self: *Evaluator,
        fa: *ast.FieldAccess,
        outer_name: []const u8,
        inner_name: []const u8,
        ...
        for (cd.members) |member| {
            switch (member) {
                ...
                .class_decl => |inner_cd| {
                    if (!std.ascii.eqlIgnoreCase(inner_cd.name, inner_name)) continue;
                    if (std.ascii.eqlIgnoreCase(fa.field, "class")) {
                        return try self.fa_make_type_object_fq(outer_name, inner_cd.name);
                    }
                    const fq_inner_name = try std.fmt.allocPrint(
                        self.arena,
                        "{s}.{s}",
                        .{ outer_name, inner_cd.name },
                    );
                ...
                .interface_decl => |inner_iface| {
                    if (std.ascii.eqlIgnoreCase(inner_iface.name, inner_name) and
                        std.ascii.eqlIgnoreCase(fa.field, "class"))
                    {
                        return try self.fa_make_type_object_fq(outer_name, inner_iface.name);
                    }
                },
                else => {},
            }
    ...
    /// Handle `System.<Enum>.X` and `Schema.<X>.Y` shortcuts.
    fn fa_nested_in_system_or_schema(
        self: *Evaluator,
        fa: *ast.FieldAccess,
        outer_name: []const u8,
        inner_name: []const u8,
        ...
        if (std.ascii.eqlIgnoreCase(fa.field, "class") and
            std.ascii.eqlIgnoreCase(outer_name, "System"))
        {
            return try self.fa_make_type_object_fq("System", inner_name);
        }
        if (std.ascii.eqlIgnoreCase(outer_name, "Schema") and
    ...
    fn fa_make_type_object_fq(
        ...
        const type_obj = try self.arena.create(types.ObjectInstance);
        type_obj.* = .{ .class_name = "Type" };
        const fq_name = try std.fmt.allocPrint(self.arena, "{s}.{s}", .{ outer, inner });
        try type_obj.fields.put(self.arena, "name", Value{ .string = fq_name });
        return Value{ .object = type_obj };
    }

    /// `.class` literal variants: multi-level dotted chain (A.B.C.class),
    /// single-identifier (ClassName.class), and string-named outer
    /// (OuterClass.InnerClass.class produced via a string value).
    fn fa_try_class_literal(
        self: *Evaluator,
        fa: *ast.FieldAccess,
        obj: Value,
    ) anyerror!?Value {
        ...
        if (fa.object.* == .identifier and std.ascii.eqlIgnoreCase(fa.field, "class")) {
            return try self.fa_class_literal_from_simple(fa.object.identifier.name);
        }
        if (obj == .string and std.ascii.eqlIgnoreCase(fa.field, "class")) {
            return try self.fa_class_literal_from_simple(obj.string);
        }
        return null;
    }

    fn fa_class_literal_from_simple(
        self: *Evaluator,
        simple_name: []const u8,
    ) anyerror!Value {
        const type_name: []const u8 = if (self.current_class) |cc| blk: {
            ...
            break :blk if (self.find_class(fq) != null)
                ...
                self.resolve_full_class_name(simple_name);
    ...
    fn eval_field_access(
        ...
        if (try self.fa_try_apexpages_schema_nested(fa)) |v| return v;
        if (try self.fa_try_schema_prefix_identifier(fa)) |v| return v;
        if (try self.fa_try_sobjecttype_shortcut(fa)) |v| return v;
        if (try self.fa_try_system_search(fa)) |v| return v;
        if (try self.fa_try_label_identifier(fa)) |v| return v;
        if (obj == .list) return try self.fa_list_access(fa, obj.list);
        if (obj == .sobject) return try self.fa_sobject_access(fa, obj.sobject);
        if (obj == .object) return try self.fa_object_instance_access(fa, obj.object);
        if (obj == .string) {
            if (try self.fa_string_access(fa, obj.string)) |v| return v;
        }
        if (try self.fa_try_null_with_static_base(fa, obj, current_env)) |v| return v;
        if (try self.fa_try_static_class_access(fa)) |v| return v;
        if (try self.fa_try_nested_static_access(fa)) |v| return v;
        if (try self.fa_try_class_literal(fa, obj)) |v| return v;
        return Value.null_val;
    }
    ...
    fn collect_dotted_identifier_chain(self: *Evaluator, expr: *const ast.Expr) ?[]const u8 {
    ...
    fn is_date_only_format_string(s: []const u8) bool {
    ...
    fn is_date_time_format_string(s: []const u8) bool {
    ...
    fn instanceof_matches_primitive(val: Value, tn: []const u8) bool {
        ...
        if (val == .string) {
            return std.ascii.eqlIgnoreCase(tn, "String") or
                (std.ascii.eqlIgnoreCase(
                    tn,
                    "Id",
                ) and Evaluator.is_salesforce_id_string(val.string));
        }
        if (val == .sobject) {
    ...
    fn is_subclass_of(self: *Evaluator, child_class: []const u8, parent_type: []const u8) bool {
        // Direct match
        ...
        if ((std.ascii.eqlIgnoreCase(parent_type, "Exception") or std.ascii.eqlIgnoreCase(
            parent_type,
            "System.Exception",
        )) and
    ...
    fn iso_date_to_epoch_days(year: i32, month: u8, day: u8) i64 {
        ...
        return (y - 1970) * 365 + @divFloor(y - 1969, 4) - @divFloor(
            y - 1901,
            100,
    ...
    fn iso_day_of_week(year: i32, month: u8, day: u8) u8 {
        ...
        const epoch_days = iso_date_to_epoch_days(year, month, day);
    ...
    fn iso_week_of_year(year: i32, month: u8, day: u8) IsoWeek {
        ...
        const is_leap_this = if ((@mod(year, 4) == 0 and (@mod(
            year,
            100,
        ) != 0 or @mod(year, 400) == 0))) true else false;
        _ = is_leap_this;
        // How many ISO weeks does this year have? Either 52 or 53.
        const jan1_dow: i32 = @as(i32, iso_day_of_week(year, 1, 1));
        const dec31_dow: i32 = @as(i32, iso_day_of_week(year, 12, 31));
    ...
    fn assert_equal(self: *Evaluator, args: []const Value) !void {
        if (self.values_equal(args[0], args[1])) return;
    ...
    fn assert_not_equal(self: *Evaluator, args: []const Value) !void {
        if (!self.values_equal(args[0], args[1])) return;
    ...
    fn assert_boolean(self: *Evaluator, args: []const Value, expected: bool) !void {
        ...
        const expected_label: []const u8 = if (expected) "true" else "false";
        self.assertion_failure = if (args.len >= 2 and args[1] == .string)
            try std.fmt.allocPrint(
                self.arena,
                "{s} | Expected: {s}, Actual: {s}",
                .{ args[1].string, expected_label, actual_str },
            )
        else
            try std.fmt.allocPrint(
                self.arena,
                "Expected: {s}, Actual: {s}",
                .{ expected_label, actual_str },
            );
    }

    fn assert_not_null(self: *Evaluator, args: []const Value) !void {
        if (args[0] != .null_val) return;
        self.assertion_failure = if (args.len >= 2 and args[1] == .string)
            try std.fmt.allocPrint(
                self.arena,
    ...
    fn assert_null(self: *Evaluator, args: []const Value) !void {
        if (args[0] == .null_val) return;
        const actual_str = try utils.coerce_to_string(args[0], self.arena);
        self.assertion_failure = if (args.len >= 2 and args[1] == .string)
            try std.fmt.allocPrint(
    ...
    fn get_dml_options_all_or_none(_: *Evaluator, value: Value) ?bool {
    ...
    fn sobject_id_for_result(_: *Evaluator, obj: *types.SObject) ?[]const u8 {
    ...
    fn create_empty_result_list_value(self: *Evaluator) !Value {
        ...
        const appendFailure = struct {
            fn build(
                ...
                const result = try self_eval.create_dml_result_value(
                    ...
                    false,
                    result_id,
                    if (op_kind == .upsert) false else null,
                );
        ...
        switch (target) {
            ...
            .sobject => {
                ...
                return switch (op) {
                    ...
                    .upsert => blk: {
                        const was_created = self.will_upsert_create_record(
                            target.sobject,
                            external_id_field,
                        );
                        try self.execute_dml_with_external_id_internal(
                ...
                for (items) |item| {
                    if (item != .sobject) {
                        try list.items.append(
                            self.arena,
                            try appendFailure.build(self, result_class, item, op),
                        );
                        continue;
                    }
                    const was_created = op == .upsert and self.will_upsert_create_record(
                        item.sobject,
                        external_id_field,
                    );
                    const result = switch (op) {
                        .insert => blk: {
                            self.execute_dml_with_external_id_internal(
                                .insert,
                                item,
                                null,
                                false,
                                ...
                                const failed = try appendFailure.build(
                                    self,
                                    result_class,
                                    item,
                                    op,
                            ...
                            break :blk try self.create_dml_result_value(
                                result_class,
                                true,
                                self.sobject_id_for_result(item.sobject),
                                null,
                            );
                        },
                        .update => blk: {
            ...
            .set => |records| {
                ...
                try list.items.append(
                    self.arena,
                    try appendFailure.build(self, result_class, target, op),
                );
                return Value{ .list = list };
            },
        }
    ...
    fn handle_database_method(
        ...
        if (is_database_dml_method(method)) {
        ...
        if (std.ascii.eqlIgnoreCase(method, "getQueryLocator")) {
            return try self.handle_database_get_query_locator(args);
        }
        ...
        if (std.ascii.eqlIgnoreCase(method, "merge")) {
            return try self.handle_database_merge(args);
        }
        return .void_val;
    }

    fn is_database_dml_method(method: []const u8) bool {
    ...
    fn database_result_class(method: []const u8) []const u8 {
    ...
    /// Extract allOrNothing from a Database.insert/update/upsert/delete call.
    /// Upsert with an external-id field uses the THIRD arg; other ops use the
    /// second. Falls back to the DmlOptions record or true.
    fn resolve_all_or_nothing(
        self: *Evaluator,
        is_upsert: bool,
        args: []const Value,
    ) bool {
    ...
    /// `args[1] == AccessLevel.USER_MODE` combined with a min-access user and
    /// no permset assignments must raise SecurityException to match real Apex.
    fn check_database_user_mode_security(
        self: *Evaluator,
        args: []const Value,
    ) !void {
        ...
        const has_permset_db = if (self.store.get("PermissionSetAssignment")) |psa|
            ...
            false;
        if (!(self.is_min_access_user and !has_permset_db)) return;
        ...
        const from_type = if (args[0] == .sobject)
            args[0].sobject.type_name
        else if (args[0] == .list and args[0].list.items.items.len > 0 and
    ...
    fn handle_database_dml_method(
        ...
        const result_class = database_result_class(method);
        ...
        const external_id_field = if (is_upsert and args.len >= 2 and args[1] != .boolean)
            ...
            null;
        const all_or_nothing = self.resolve_all_or_nothing(is_upsert, args);
        try self.check_database_user_mode_security(args);

        var upsert_creates: std.ArrayListUnmanaged(bool) = .empty;
        if (is_upsert and args.len > 0) {
            try self.precompute_upsert_creates(&upsert_creates, args[0], external_id_field);
        }

        if (args.len > 0) {
            const op = database_dml_op(method);
            if (try self.invoke_database_dml(
                op,
                result_class,
                is_upsert,
                all_or_nothing,
        ...
        return try self.build_database_success_results(
            result_class,
            is_upsert,
            args,
    ...
    fn precompute_upsert_creates(
        self: *Evaluator,
        upsert_creates: *std.ArrayListUnmanaged(bool),
        target: Value,
        external_id_field: ?[]const u8,
    ) !void {
        if (target == .sobject) {
            try upsert_creates.append(
                self.arena,
        ...
        if (target != .list) return;
        for (target.list.items.items) |item| {
            try upsert_creates.append(self.arena, if (item == .sobject)
    ...
    /// Run the DML. In allOrNothing mode exceptions propagate; in best-effort
    /// mode we return a failure result value directly, nulling the pending
    /// exception so the caller sees the result list instead.
    fn invoke_database_dml(
        self: *Evaluator,
        op: ast.DmlOp,
        result_class: []const u8,
        is_upsert: bool,
    ...
    fn build_database_success_results(
        self: *Evaluator,
        result_class: []const u8,
        is_upsert: bool,
        args: []const Value,
        ...
        if (args.len > 0 and args[0] == .sobject) {
            ...
            return Value{ .object = try self.make_dml_success_result(
                result_class,
        ...
        if (args.len > 0 and args[0] == .list) {
            for (args[0].list.items.items, 0..) |item, idx| {
                const was_created = is_upsert and
                    (if (idx < upsert_creates.len) upsert_creates[idx] else true);
                const record_id = if (item == .sobject)
                    item.sobject.id orelse "001000000000001"
                else
                    "001000000000001";
                try list.items.append(self.arena, Value{ .object = try self.make_dml_success_result(
                    result_class,
                    record_id,
                    is_upsert,
                    was_created,
    ...
    fn handle_database_query_with_binds(
        ...
        if (!(args.len >= 2 and args[0] == .string)) return try self.make_empty_list();
            ...
            while (pos < soql_str.len) {
                if (soql_str[pos] == ':') {
                    ...
                    while (end < soql_str.len and
                        (std.ascii.isAlphanumeric(soql_str[end]) or
                            soql_str[end] == '_')) : (end += 1)
                    {}
    ...
    fn handle_database_count_query(
        ...
        if (!(args.len > 0 and args[0] == .string)) return Value{ .integer = 0 };
    ...
    fn handle_database_get_query_locator(
        self: *Evaluator,
        args: []const Value,
    ) !Value {
    ...
    /// Database.executeBatch runs in a separate transaction; save/restore the
    /// active transaction's governor-limit counters across the call.
    fn handle_database_execute_batch(
        self: *Evaluator,
        args: []const Value,
    ) !Value {
        ...
        if (self.batch_job_runner_active or self.batch_lifecycle_depth > 0) {
            return try self.enqueue_nested_batch_job(args[0], obj);
        }
        return try self.run_standalone_batch_job(args[0], obj);
    }

    fn enqueue_nested_batch_job(
        self: *Evaluator,
        job_value: Value,
        obj: *types.ObjectInstance,
    ) !Value {
        const job_id = try self.create_async_apex_job("BatchApex", obj.class_name, "execute");
        try obj.fields.put(self.arena, "__batchJobId", Value{ .string = job_id });
        ...
        if (self.batch_job_runner_active) {
            try self.pending_batch_jobs.append(self.arena, job_value);
        }
        return Value{ .string = job_id };
    }

    fn run_standalone_batch_job(
        self: *Evaluator,
        job_value: Value,
        obj: *types.ObjectInstance,
    ) !Value {
    ...
    /// Database.merge(primary, secondaries, allOrNothing): delete the
    /// secondaries from the store, cascade-remove DuplicateRecordItems that
    /// reference them, and clean up orphaned DuplicateRecordSets.
    fn handle_database_merge(self: *Evaluator, args: []const Value) !Value {
        if (args.len >= 2) {
            var secondary_ids: std.ArrayListUnmanaged([]const u8) = .empty;
            try self.collect_merge_secondary_ids(args[1], &secondary_ids);
            self.delete_merge_secondaries(secondary_ids.items);
            self.cascade_delete_duplicate_record_items(secondary_ids.items);
        }
    ...
    fn collect_merge_secondary_ids(
        self: *Evaluator,
        secondaries: Value,
        out: *std.ArrayListUnmanaged([]const u8),
    ) !void {
        if (secondaries == .sobject and secondaries.sobject.id != null) {
            try out.append(self.arena, secondaries.sobject.id.?);
            return;
        }
        if (secondaries != .list) return;
        for (secondaries.list.items.items) |item| {
            if (item == .sobject and item.sobject.id != null) {
                try out.append(self.arena, item.sobject.id.?);
            }
        }
    }

    fn delete_merge_secondaries(self: *Evaluator, ids: []const []const u8) void {
        for (ids) |sec_id| {
            var store_iter = self.store.iterator();
            while (store_iter.next()) |entry| {
                var i: usize = 0;
                while (i < entry.value_ptr.items.len) {
                    const rec = entry.value_ptr.items[i];
                    if (rec == .sobject and rec.sobject.id != null and
                        std.mem.eql(u8, rec.sobject.id.?, sec_id))
                    {
    ...
    fn cascade_delete_duplicate_record_items(
        self: *Evaluator,
        secondary_ids: []const []const u8,
    ) void {
        const dri_records = self.store.getPtr("DuplicateRecordItem") orelse return;
        var i: usize = 0;
        while (i < dri_records.items.len) {
            ...
            const rec_id_val = utils.sobject_get(&item.sobject.fields, "RecordId") orelse {
                i += 1;
                continue;
            };
            if (rec_id_val != .string) {
                i += 1;
                continue;
            }
    ...
    fn handle_test_factory(
        ...
        if (std.ascii.eqlIgnoreCase(class_name, "TestFactory")) {
            return try self.handle_test_factory_namespace(method_name, args);
        }
        if (std.ascii.eqlIgnoreCase(class_name, "TestDataHelpers")) {
            return try self.handle_test_data_helpers_namespace(method_name, args);
        }
        if (std.ascii.eqlIgnoreCase(class_name, "TestHelper")) {
            return try self.handle_test_helper_namespace(method_name, args);
        }
        return null;
    }

    /// Check whether a user-defined test-factory class overrides the given
    /// method. When it does, return true so the caller can bail and let the
    /// normal dispatcher invoke the user method.
    fn user_test_factory_overrides(
        self: *Evaluator,
        class_name: []const u8,
        method_name: []const u8,
        args: []const Value,
    ...
    fn handle_test_factory_namespace(
        self: *Evaluator,
        method_name: []const u8,
        args: []const Value,
    ) !?Value {
        if (std.ascii.eqlIgnoreCase(method_name, "createSObject")) {
            return try self.test_factory_create_sobject(args);
        }
        if (std.ascii.eqlIgnoreCase(method_name, "createSObjectList")) {
            return try self.test_factory_create_sobject_list(args);
        }
        if (std.ascii.eqlIgnoreCase(method_name, "assignPermSetToUser")) {
            if (self.user_test_factory_overrides("TestFactory", method_name, args)) return null;
            return .void_val;
        }
        ...
        {
            if (self.user_test_factory_overrides("TestFactory", method_name, args)) return null;
            return try self.test_factory_create_user_stub(method_name, args);
        }
        return null;
    }

    fn test_factory_create_sobject(
        self: *Evaluator,
        args: []const Value,
    ) !?Value {
        if (args.len == 0 or args[0] != .sobject) return Value.null_val;
        const sobj = args[0].sobject;
        if (utils.sobject_get(&sobj.fields, "Name") == null) {
            try sobj.fields.put(self.arena, "Name", Value{ .string = "Test Record" });
        }
        if (std.ascii.eqlIgnoreCase(sobj.type_name, "Contact") and
            utils.sobject_get(&sobj.fields, "LastName") == null)
        {
            try sobj.fields.put(self.arena, "LastName", Value{ .string = "Test Record" });
        }
        // Second arg: boolean true → insert; string → defaults-class name → also insert.
        if (args.len >= 2 and args[1] == .boolean and args[1].boolean) try self.insert_record(sobj);
        if (args.len >= 2 and args[1] == .string) try self.insert_record(sobj);
        return args[0];
    }

    fn test_factory_create_sobject_list(
        self: *Evaluator,
        args: []const Value,
    ) !?Value {
    ...
    fn test_factory_create_user_stub(
        self: *Evaluator,
        method_name: []const u8,
        args: []const Value,
    ) !Value {
        ...
        const profile = try self.arena.create(types.SObject);
        profile.* = .{ .type_name = "Profile" };
        const profile_name: []const u8 = if (std.ascii.eqlIgnoreCase(
            method_name,
            "createMinAccessUser",
        ))
    ...
    fn handle_test_data_helpers_namespace(
        self: *Evaluator,
        method_name: []const u8,
        args: []const Value,
    ) !?Value {
        if (self.user_test_factory_overrides("TestDataHelpers", method_name, args)) return null;
        ...
        if (std.ascii.eqlIgnoreCase(method_name, "genAccountWithOptions")) {
            ...
            acct.* = .{ .type_name = "Account" };
            try acct.fields.put(self.arena, "Name", Value{ .string = "Awesome Test Account" });
            try acct.fields.put(self.arena, "ShippingStreet", Value{
                .string = "123 Sessame St.",
            });
    ...
    fn handle_test_helper_namespace(
        self: *Evaluator,
        method_name: []const u8,
        args: []const Value,
    ) !?Value {
        if (self.user_test_factory_overrides("TestHelper", method_name, args)) return null;
            ...
            switch (member) {
                .constructor_decl => |cd| {
                    if (cd.params.len == 0) {
                        ...
                        _ = self.exec_block(cd.body, ctor_env) catch {};
                        break;
                    }
                },
                else => {},
    ...
    pub fn resolve_full_class_name_public(self: *Evaluator, name: []const u8) []const u8 {
    ...
    pub fn value_to_string_public(self: *Evaluator, value: Value) anyerror![]const u8 {
        return self.value_to_string(value);
    }

    pub fn value_hash_code_public(self: *Evaluator, value: Value) anyerror!i64 {
        return self.value_hash_code(value);
    }

    pub fn values_equal_public(self: *Evaluator, left: Value, right: Value) bool {
    ...
    fn value_to_string(self: *Evaluator, value: Value) anyerror![]const u8 {
    ...
    fn string_hash_code(_: *Evaluator, value: []const u8) i64 {
        var result: i32 = 0;
        for (value) |ch| {
    ...
    fn strict_values_equal(self: *Evaluator, left: Value, right: Value) bool {
        _ = self;
    ...
    fn value_hash_code(self: *Evaluator, value: Value) anyerror!i64 {
        return switch (value) {
            ...
            .long => |i| i,
            .double => |d| @intFromFloat(d),
            .string => |s| self.string_hash_code(s),
            .sobject => |sob| blk: {
                if (sob.id) |id| break :blk self.string_hash_code(id);
                const json = try utils.to_json(value, self.arena);
    ...
    fn find_resolved_method_in_hierarchy_typed(
        ...
        while (current) |cd| {
            if (cd.super_class) |sc| {
                const parent = self.find_class(sc.name) orelse break;
    ...
    fn find_resolved_method_in_hierarchy(
        ...
        if (actual_class) |ac| {
            var cur: ?*ast.ClassDecl = ac;
            while (cur) |cd| {
                if (self.find_method_in_class(
                    cd,
                    method_name,
                    arg_count,
                )) |md| return .{ .owner = cd, .method = md };
                cur = if (cd.super_class) |sc| self.find_class(sc.name) else null;
            }
        }
        if (self.find_method_in_class(
            ...
            method_name,
            arg_count,
        )) |md| return .{ .owner = class_decl, .method = md };
        var current: ?*ast.ClassDecl = class_decl;
        while (current) |cd| {
            if (cd.super_class) |sc| {
    ...
    /// Type-aware version of find_method_in_hierarchy. Walks the runtime (child)
    ...
    fn find_compatible_method_in_class(
        ...
        for (candidates[0..count]) |md| {
            if (self.single_method_candidate_score(md, args, arg_type_hints)) |score| {
                if (best == null or score > best_score) {
                    best = md;
                    best_score = score;
                }
    ...
    fn enum_access_type_name(self: *Evaluator, expr: *const ast.Expr) ?[]const u8 {
    ...
    fn string_matches_enum_param_type(
        ...
        const simple = if (std.mem.lastIndexOfScalar(
            u8,
            param_type,
            '.',
        )) |di| param_type[di + 1 ..] else param_type;
        ...
        if (self.find_visible_enum_decl(simple)) |ed| {
            for (ed.values) |v| {
    ...
    fn overload_score_for_type_hint(
        ...
        const hint_base = type_base_name(hint);
        const pt_base = type_base_name(pt);
        ...
        if (is_collection_type_name(hint_base)) {
            return score_collection_vs_iterable(
                hint,
                pt,
                hint_base,
                pt_base,
        ...
        for (class_decl.members) |member| {
            switch (member) {
                .field_decl => |fd| {
                    if (!fd.modifiers.is_static) {
                        const val = if (fd.initializer) |init_expr|
                            self.eval_expr(init_expr, init_env) catch Value.null_val
                        else
                            default_value(fd.type_ref);
    ...
    fn class_has_no_arg_ctor(class_decl: *ast.ClassDecl) bool {
    ...
    fn ctor_starts_with_super_or_this(cd: *ast.ConstructorDecl) bool {
    ...
    fn score_constructor_arg(self: *Evaluator, arg: Value, pt: []const u8) i32 {
        ...
        if (arg == .integer and
            (std.ascii.eqlIgnoreCase(
                pt,
                "Integer",
            ) or std.ascii.eqlIgnoreCase(pt, "int"))) return 2;
    ...
    fn is_instance_field(_: *Evaluator, class_decl: *ast.ClassDecl, name: []const u8) bool {
        for (class_decl.members) |member| {
            switch (member) {
                .field_decl => |fd| {
                    if (!fd.modifiers.is_static and std.ascii.eqlIgnoreCase(fd.name, name))
                        return true;
                },
                else => {},
            }
    ...
    fn find_field_decl_with_owner(
        ...
        while (current) |cd| {
            ...
            current = if (cd.super_class) |sc| self.find_class(sc.name) else null;
        }
        return null;
    }
    ...
    fn is_parent_instance_field(
        ...
        if (class_decl.super_class) |sc| {
            if (self.find_class(sc.name)) |parent| {
                ...
                return self.is_parent_instance_field(parent, name);
            }
        }
        return false;
    }

    pub fn find_class_public(self: *Evaluator, name: []const u8) ?*ast.ClassDecl {
        return self.find_class(name);
    }

    pub fn instantiate_class_public(self: *Evaluator, class_name: []const u8) !Value {
        return self.instantiate_class(class_name);
    }

    /// True for a bare SObject simple name (e.g. "Account", "MyObject__c").
    /// Public wrapper around the internal `is_s_object_type_name` used by builtin dispatch.
    pub fn is_s_object_type_name_public(self: *Evaluator, name: []const u8) bool {
        return self.is_s_object_type_name(name);
    }
    ...
    fn create_async_apex_job(
        ...
        const apex_class_name = if (std.mem.lastIndexOfScalar(
            u8,
            top_level_class,
            '.',
        )) |di| top_level_class[di + 1 ..] else top_level_class;
        const apex_class = try self.arena.create(types.SObject);
        apex_class.* = .{ .type_name = "ApexClass" };
        try apex_class.fields.put(
            self.arena,
            "Id",
            Value{ .string = try std.fmt.allocPrint(self.arena, "01p{d:0>15}", .{self.next_id}) },
        );
    ...
    fn build_queueable_context(self: *Evaluator, job_id: []const u8) !Value {
        const ctx = try self.arena.create(types.ObjectInstance);
    ...
    fn enqueue_job(self: *Evaluator, job_obj: *types.ObjectInstance) !Value {
        ...
        const job_id = try self.create_async_apex_job("Queueable", job_obj.class_name, null);
        self.attached_finalizer = null;
        ...
        if (self.find_class(job_obj.class_name)) |job_class| {
            ...
            _ = execute_result catch |err| {
                if (err == error.ApexException) {
                    ...
                    self.update_async_apex_job(job_id, "Failed", 1);
                    try self.invoke_attached_finalizer(job_id, thrown_exception);
    ...
    fn compare_by_field(self: *Evaluator, a: Value, b: Value, field: []const u8) i32 {
        const av = if (a == .sobject) self.get_s_object_field_value_case_insensitive(
            a.sobject,
            field,
        ) orelse Value.null_val else Value.null_val;
        const bv = if (b == .sobject) self.get_s_object_field_value_case_insensitive(
            b.sobject,
            field,
        ) orelse Value.null_val else Value.null_val;
        // null sorts last
        ...
        if (av == .string and bv == .string) {
            return compare_case_insensitive_strings(av.string, bv.string);
        }
    ...
    fn compare_values(_: *Evaluator, a: Value, b: Value) i32 {
        // null handling
        if (a == .null_val and b == .null_val) return 0;
        if (a == .null_val) return -1;
        if (b == .null_val) return 1;
        if (builtins.extract_date_string(a)) |a_date| {
            if (builtins.extract_date_string(b)) |b_date| {
                return compare_case_insensitive_strings(a_date, b_date);
            }
        }
        // String comparison
        if (a == .string and b == .string) {
            return compare_case_insensitive_strings(a.string, b.string);
        }
        // Integer/Long comparison
    ...
    fn compare_case_insensitive_strings(left: []const u8, right: []const u8) i32 {
    ...
    fn class_implements_interface(
        ...
        // Check parent class
        if (cd.super_class) |sc| {
            return self.class_implements_interface(sc.name, interface_name);
        }
        return false;
    }

    fn has_comparable_interface(self: *Evaluator, class_name: []const u8) bool {
    ...
    fn is_batch_lifecycle_method(
        ...
        return self.class_implements_interface(class_name, "Database.Batchable") or
            self.class_implements_interface(class_name, "Batchable");
    }
    ...
    fn parse_json_value(self: *Evaluator, json_str: []const u8, type_hint: []const u8) ?Value {
        const trimmed = std.mem.trim(u8, json_str, " \t\r\n");
        if (trimmed.len == 0) return null;
        if (trimmed[0] == '[') return self.parse_json_array(trimmed, type_hint);
        if (trimmed[0] == '{') return self.parse_json_object(trimmed, type_hint);
        return parse_json_scalar(self, trimmed, type_hint);
    }

    fn parse_json_scalar(self: *Evaluator, trimmed: []const u8, type_hint: []const u8) ?Value {
        if (trimmed[0] == '"') {
            const end = std.mem.lastIndexOfScalar(u8, trimmed, '"') orelse
                return Value{ .string = "" };
            if (end == 0) return Value{ .string = "" };
    ...
    fn parse_json_array(self: *Evaluator, trimmed: []const u8, type_hint: []const u8) ?Value {
        ...
        while (i < trimmed.len) : (i += 1) {
            if (trimmed[i] == '"') {
                ...
                if (depth == 0) {
                    ...
                    if (elem_str.len > 0) {
                        if (self.parse_json_value(elem_str, elem_type)) |v| {
    ...
    fn parse_json_object(self: *Evaluator, trimmed: []const u8, type_hint: []const u8) ?Value {
        if (is_framework_api_object_type(type_hint)) {
            return self.parse_json_generic_object(trimmed, type_hint);
        }
        if (self.find_class(type_hint) != null) {
            return self.parse_json_user_class_object(trimmed, type_hint);
        }
        return self.parse_json_sobject(trimmed, type_hint);
    }

    /// Framework classes like Invocable.Action.Result / Flow.* / Messaging.* /
    /// Auth.* / Reports.* get parsed into ObjectInstance values so the
    /// class-specific dispatch in eval_instance_method can handle getters.
    fn is_framework_api_object_type(type_hint: []const u8) bool {
        return std.mem.startsWith(u8, type_hint, "Invocable.") or
    ...
    fn parse_json_generic_object(
        ...
        while (ji < trimmed.len) : (ji += 1) {
            if (trimmed[ji] == '"' and jd == 0) {
                const parsed = parse_json_object_key_value(trimmed, ji);
                if (self.parse_json_value(parsed.val_str, "Object")) |v| {
                    obj.fields.put(self.arena, parsed.key, v) catch {};
                }
                ji = parsed.next;
    ...
    fn parse_json_user_class_object(
        self: *Evaluator,
        trimmed: []const u8,
        type_hint: []const u8,
    ) ?Value {
        const obj = self.arena.create(types.ObjectInstance) catch return null;
        obj.* = .{ .class_name = type_hint };
        if (self.find_class(type_hint)) |class_decl| {
            if (class_decl.super_class) |sc| {
                if (self.find_class(sc.name)) |parent_decl| {
                    self.init_instance_fields(parent_decl, obj) catch {};
                }
            }
            self.init_instance_fields(class_decl, obj) catch {};
        }
        var jd: i32 = 0;
        var ji: usize = 1;
        while (ji < trimmed.len) : (ji += 1) {
            if (trimmed[ji] == '"' and jd == 0) {
                const parsed = parse_json_object_key_value(trimmed, ji);
                const field_type_hint = self.resolve_user_class_field_type(
                    type_hint,
                    parsed.key,
                );
                if (self.parse_json_value(parsed.val_str, field_type_hint)) |v| {
                    obj.fields.put(self.arena, parsed.key, v) catch {};
                }
                ji = parsed.next;
            } else if (trimmed[ji] == '{' or trimmed[ji] == '[') {
    ...
    /// Starting at `start_quote` (pointing at `"`), parse `"key": <value>,?`
    /// and return the key, the trimmed value text, and the resume position.
    /// The returned positions are 0-based indices into `trimmed`.
    fn parse_json_object_key_value(trimmed: []const u8, start_quote: usize) ParsedKeyValue {
        const key_start = start_quote + 1;
        var ji = start_quote + 1;
        while (ji < trimmed.len and trimmed[ji] != '"') : (ji += 1) {}
        const key = trimmed[key_start..ji];
        ji += 1;
        while (ji < trimmed.len and trimmed[ji] != ':') : (ji += 1) {}
        ji += 1;
        ...
        return .{ .key = key, .val_str = val_str, .next = ji };
    }

    /// Resolve a field's declared type hint for JSON parsing. For List<T>
    /// fields, walk the class source to recover `T` (the AST stores only the
    /// container type name). Returns "Object" when no match is found.
    fn resolve_user_class_field_type(
        self: *Evaluator,
        type_hint: []const u8,
        key_name: []const u8,
    ) []const u8 {
        const cd = self.find_class(type_hint) orelse return "Object";
        for (cd.members) |member| {
            switch (member) {
                .field_decl => |fd| {
                    ...
                    if (std.ascii.eqlIgnoreCase(fd.type_ref.name, "List")) {
                        for (self.class_sources.keys(), self.class_sources.values()) |k, src| {
                            ...
                            const elem = self.find_field_array_type(src, key_name) orelse continue;
                            if (elem.len == 0 or elem.len >= 200) continue;
                            return std.fmt.allocPrint(
                                self.arena,
                                "{s}[]",
                                .{elem},
    ...
    fn parse_json_sobject(self: *Evaluator, trimmed: []const u8, type_hint: []const u8) ?Value {
        ...
        var i: usize = 1;
        while (i < trimmed.len) {
            while (i < trimmed.len and (trimmed[i] == ' ' or trimmed[i] == '\t' or
            ...
            {}
            if (i >= trimmed.len) break;
            i = self.parse_json_sobject_value(
                sob,
                trimmed,
                i,
                key,
    ...
    /// Consume one value from `trimmed` starting at `i` and store it on
    /// `sob` under `key`. Returns the index just past the parsed value.
    fn parse_json_sobject_value(
        self: *Evaluator,
        sob: *types.SObject,
        trimmed: []const u8,
        start: usize,
    ...
    fn parse_json_sobject_number(
        ...
        var num_end = start;
        while (num_end < trimmed.len and
            trimmed[num_end] != ',' and trimmed[num_end] != '}' and
            trimmed[num_end] != ' ') : (num_end += 1)
        {}
        const num_str = trimmed[start..num_end];
    ...
    fn parse_json_sobject_attributes(
        _: *Evaluator,
        sob: *types.SObject,
        nested: []const u8,
        resolved_type: *[]const u8,
    ) !void {
        const type_key_patterns = [_][]const u8{
            "\"type\":\"",
            "\"type\": \"",
            "\"type\" : \"",
        };
    ...
    fn is_known_schema_enum_type(name: []const u8) bool {
    ...
    fn instantiate_class(self: *Evaluator, class_name: []const u8) !Value {
        ...
        if (self.find_class(class_name)) |class_decl| {
            ...
            if (class_decl.super_class) |sc| {
                if (self.find_class(sc.name)) |parent_decl| {
                    self.init_instance_fields(parent_decl, instance) catch {};
                }
            }
            ...
            try self.run_constructor(class_decl, instance, &.{});
            return Value{ .object = instance };
        }
...
fn eval_binary(
    ...
    const TemporalComparable = struct {
        ...
        fn from_value(value: Value) ?[]const u8 {
            return switch (value) {
                .object => |obj| blk: {
                    if (!std.ascii.eqlIgnoreCase(obj.class_name, "Date") and
    ...
    switch (op) {
        .eq => return .{ .boolean = eval.values_equal(left, right) },
    ...
    if ((left == .double or left == .integer or left == .long) and
        (right == .double or right == .integer or right == .long))
    {
        const l = if (left == .double) left.double else if (left == .integer) @as(
            f64,
            @floatFromInt(left.integer),
        ) else @as(f64, @floatFromInt(left.long));
        const r = if (right == .double) right.double else if (right == .integer) @as(
            f64,
            @floatFromInt(right.integer),
        ) else @as(f64, @floatFromInt(right.long));
        return switch (op) {
            .add => .{ .double = l + r },
            .sub => .{ .double = l - r },
            .mul => .{ .double = l * r },
    ...
    if (op == .add and (left == .string or right == .string)) {
        const ls = try eval.value_to_string(left);
        const rs = try eval.value_to_string(right);
        const result = try std.fmt.allocPrint(arena, "{s}{s}", .{ ls, rs });
        return .{ .string = result };
    }

    if (TemporalComparable.from_value(left)) |lv| {
        if (TemporalComparable.from_value(right)) |rv| {
    ...
    if (left == .object and right == .object) {
        ...
        if ((std.ascii.eqlIgnoreCase(left_cn, "Date") or std.ascii.eqlIgnoreCase(
            left_cn,
            "Datetime",
        )) and
            (std.ascii.eqlIgnoreCase(right_cn, "Date") or std.ascii.eqlIgnoreCase(
                right_cn,
                "Datetime",
            )))
        {
...
fn eval_compound_assign(
    ...
    const Numeric = struct {
        ...
        fn from_division(current_value: Value, incoming_value: Value, result: f64) Value {
            if (current_value == .double or incoming_value == .double) {
                if (is_integral(result)) return .{ .integer = @intFromFloat(result) };
                return .{ .double = result };
            }
            if (current_value == .long or incoming_value == .long) {
                if (is_integral(result)) return .{ .long = @intFromFloat(result) };
                return .{ .double = result };
            }
            if (is_integral(result)) return .{ .integer = @intFromFloat(result) };
            return .{ .double = result };
...
fn default_value(type_ref: types.TypeRef) Value {
...
fn extract_from_type(soql: []const u8) ?[]const u8 {
...
fn is_soql_whitespace(c: u8) bool {
    return c == ' ' or c == '\t' or c == '\n' or c == '\r';
}

fn extract_where_clause(soql: []const u8) ?[]const u8 {
    ...
    while (i + 5 < soql.len) : (i += 1) {
        ...
        if (std.ascii.eqlIgnoreCase(soql[i .. i + 5], "where") and
            (i == 0 or is_soql_whitespace(soql[i - 1])) and
            is_soql_whitespace(soql[i + 5]))
        {
            const start = i + 6;
...
fn extract_offset(soql: []const u8) ?usize {
    ...
    while (i + 7 < soql.len) : (i += 1) {
        if (std.ascii.eqlIgnoreCase(soql[i .. i + 6], "offset") and
            (soql[i + 6] == ' ' or soql[i + 6] == '\n'))
        {
            var start = i + 7;
            ...
            (soql[i + 6] == ' ' or soql[i + 6] == '\n'))
        {
            ...
            if (start < soql.len and soql[start] == ':') {
                ...
                while (end < soql.len and
                    (std.ascii.isAlphanumeric(soql[end]) or soql[end] == '_')) end += 1;
...
fn extract_order_by_field(soql: []const u8) ?OrderByInfo {
    var i: usize = 0;
...
fn extract_limit_bind_var(soql: []const u8) ?[]const u8 {
    ...
    while (i + 6 < soql.len) : (i += 1) {
        if (std.ascii.eqlIgnoreCase(soql[i .. i + 5], "limit") and
            ...
            if (inner_query.len > 6 and std.ascii.eqlIgnoreCase(inner_query[0..6], "SELECT")) {
                if (std.ascii.indexOfIgnoreCase(inner_query, "FROM")) |from_pos| {
                    ...
                    if (end > start) {
                        ...
                        const rel = if (std.mem.lastIndexOfScalar(
                            u8,
                            raw_rel,
                            '.',
                        )) |dot_pos| raw_rel[dot_pos + 1 ..] else raw_rel;
...
fn eval_source(
    ...
    const result = try eval.call_method(class_name, method_name, &.{});
...
test "evaluate 1 + 2" {
    ...
    const expr = try parser_mod.parse_expr(tokens, arena.allocator());
    var eval = try Evaluator.init(arena.allocator(), std.testing.io);
    const result = try eval.eval_expr(expr, eval.global_env);
...
test "evaluate class method returning string" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "Hello", "greet");
    defer r.deinit();
...
test "evaluate class method with arithmetic" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "Calc", "add");
    defer r.deinit();
...
test "evaluate System.debug" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "Debug", "run");
    defer r.deinit();
...
test "evaluate if-else" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "Branch", "check");
    defer r.deinit();
...
test "evaluate for loop" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "Loop", "sum");
    defer r.deinit();
...
test "evaluate string concatenation" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "Concat", "run");
    defer r.deinit();
...
test "evaluate ternary expression" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "Ternary", "run");
    defer r.deinit();
...
test "inner class accesses outer class static field" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "Outer", "run");
    defer r.deinit();
...
test "SOQL assignment unwraps for previously declared SObject variable" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "SoqlAssign", "run");
    defer r.deinit();
...
test "mock singleton override feeds static lazy getter" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "Runner", "run");
    defer r.deinit();
...
test "typed null identifier prefers sobject overload over id overload" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "OverloadProbe", "run");
    defer r.deinit();
...
test "typed null identifier prefers iterable overload over object overload" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "IterableProbe", "run");
    defer r.deinit();
...
test "null-returning helper method preserves declared return type for overload hints" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "MatchLikeProbe", "run");
    defer r.deinit();
...
test "createStub forwards declared param types for typed null helper arguments" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "StubProbe", "run");
    defer r.deinit();
...
test "List.add with explicit index inserts the provided value" {
    const source =
        ...
        \\}
    ;
    var r = try eval_source(source, "IndexedListInsertProbe", "run");
    defer r.deinit();
